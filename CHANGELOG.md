...
- **Fixed Redis cache key slug/UUID mismatch (`backend/lib/cacheMiddleware.js`):** `cacheList` was building its Redis key from `req.query?.tenant_id` (the raw text slug sent by the frontend, e.g. `"acme"`) while `invalidateCache` was keying off `req.tenant?.id` (the canonical UUID resolved by `validateTenantAccess`, e.g. `"abc-123-..."`). The patterns never matched, so PUT mutations never actually evicted the cached GET response. Both `cacheList` and `cacheDetail` now prefer `req.tenant?.id` first, matching the `invalidateCache` order. This fixes accounts (and any other entity cached via `cacheList`) showing stale data after edits until the TTL expired.
