...
- **Booking shortlink `::1` localhost hardening:** Added IPv6 loopback `::1` (and optionally kept `[::1]`) to `LOCAL_HOSTS` set in `backend/routes/booking-shortlink.js` so shortlinks created from an IPv6 localhost environment are also canonicalized to the public scheduler URL.
- **Booking shortlink production startup guard:** Added fail-fast validation of `PUBLIC_SCHEDULER_URL` in `backend/server.js` — if the env var is not a valid `https://` URL with proper credentials check in production, the server refuses to start, preventing silent shortlink misconfiguration.
- **Canonicalization test env isolation:** Fixed `backend/__tests__/routes/booking-shortlink.canonicalization.test.js` tests to restore `process.env.PUBLIC_SCHEDULER_URL` properly after each test, eliminating environment leakage between test runs.
