...
- **Coverage backlog artifact:** Added `docs/developer-docs/COVERAGE_BACKLOG.md` generated from `scripts/test-alignment-report.js --only-gaps` to prioritize high-impact test coverage work by module area.
- **New backend unit tests:** Added `backend/__tests__/lib/urlValidator.test.js` (SSRF URL validation checks, whitelist behavior, internal-host enforcement) and `backend/__tests__/lib/typeConversions.test.js` (nullable/string/numeric/integer/boolean conversion utilities and assign helper semantics).
- **Additional backend unit test batch:** Added `backend/__tests__/lib/aiBudgetConfig.test.js` (env override + bounds/clamping behavior), `backend/__tests__/lib/tokenBudget.test.js` (token estimation, budget report, cap enforcement/drop-order behavior), and `backend/__tests__/lib/validation.test.js` (validation module UUID re-export contract).
- **C.A.R.E. and agent charter coverage batch:** Added `backend/__tests__/lib/agentCharters.test.js`, `backend/__tests__/lib/isCareAutonomyEnabled.test.js`, and `backend/__tests__/lib/isCareStateWriteEnabled.test.js` to lock charter retrieval behavior and safety gate semantics for autonomy/state-write env flags.
- **Request/status mapping coverage batch:** Added `backend/__tests__/lib/requestContext.test.js` (AsyncLocalStorage DB timing accumulation + readback behavior) and `backend/__tests__/lib/statusCardLabelResolver.test.js` (status label map construction and tool-arg normalization behavior).
- **C.A.R.E. pure-module coverage batch:** Added `backend/lib/care/__tests__/careTypes.test.js`, `backend/lib/care/__tests__/careEscalationTypes.test.js`, `backend/lib/care/__tests__/careAuditTypes.test.js`, and `backend/lib/care/__tests__/careEscalationLexicon.test.js` to validate immutable exports, guard defaults, audit-event shaping, normalization, and phrase-match helpers in core C.A.R.E. utility modules.
- **Tenant integrations list-filter regression tests:** Added `backend/__tests__/routes/tenant-integrations.list-filter.test.js` covering all list paths: omitted `is_active`, `is_active=true`, and `is_active=false`.
- **Shortlink canonicalization regression tests:** Added `backend/__tests__/routes/booking-shortlink.canonicalization.test.js` covering localhost rewrite to scheduler domain, unchanged scheduler URLs, and malformed/unsafe URL rejection.
- **Cal.com production hardening runbook:** Added `docs/developer-docs/CALCOM_PRODUCTION_HARDENING.md` with build-time env requirements, immutable tagging guidance, post-deploy verification commands, and rollback steps.
...
- **Test Alignment coverage-gap matching improved:** Expanded `findTestFile()` in `scripts/test-alignment-report.js` to normalize path separators and recognize broader repository test conventions (backend routes/lib/services/middleware/workers/utils, frontend `src/components`/`src/pages`/`src/hooks`/`src/api` mirrors, and dotted variant tests). This reduces false-positive "No test file" gaps and makes remaining coverage gaps more actionable.
- **Test Alignment backend co-located test discovery:** Updated `scripts/test-alignment-report.js` test scan patterns to include `backend/**/__tests__/**/*.js` and `backend/**/*.test.js`, so co-located backend library tests (including `backend/lib/care/__tests__`) are counted during coverage-gap analysis instead of being reported as false gaps.
- **Test Alignment orphan detection generalized:** Updated `scripts/test-alignment-report.js` to reduce false-positive orphan reports by normalizing Windows path separators, expanding source scan coverage (`backend/middleware/**`, `backend/workers/**`), and adding generalized test-to-source mapping rules for backend routes/services/utils/workers, frontend `src/__tests__` and `src/**/__tests__` naming variants, and `tests/**/*.spec.*` E2E suites. This resolves bulk false positives that previously required one-off mappings and now reports zero orphaned tests for the current repository state.
- **Removed stale duplicate route tests:** Deleted `backend/__tests__/routes/contacts.route.test.FIXED.js` and `backend/__tests__/routes/leads.route.test.FIXED.js` (staging artifacts duplicated by canonical `.test.js` files).
...
- **Cal.com production OAuth/env wiring:** Updated `docker-compose.prod.yml` so Cal.com uses `NEXTAUTH_URL=${CALCOM_NEXTAUTH_URL:-https://scheduler.aishacrm.com}` (instead of localhost) and added `GOOGLE_API_CREDENTIALS=${CALCOM_GOOGLE_API_CREDENTIALS:-{}}` to avoid startup JSON parse failures when Google creds are unset. Updated `.github/workflows/docker-release.yml` to inject `CALCOM_GOOGLE_API_CREDENTIALS` from Doppler when available.
- **Tenant integrations list behavior:** Updated `backend/routes/tenant-integrations.js` GET list route so the `is_active` filter is only applied when query param `is_active` is explicitly provided (`is_active !== null`). Omitted `is_active` now returns both active and inactive rows.
- **Booking shortlink canonicalization:** Updated `backend/routes/booking-shortlink.js` to normalize destination URLs before persistence by rewriting `localhost`/`127.0.0.1` origins to `PUBLIC_SCHEDULER_URL`, rejecting malformed URLs, non-http(s) schemes, and credential-bearing URLs with explicit 400 errors.
- **Frontend scheduler URL source hardening:** Updated `src/components/employees/EmployeeForm.jsx`, `src/components/scheduling/BookingWidget.jsx`, and `src/components/settings/CalendarSync.jsx` to use one build-time source (`VITE_CALCOM_URL`) via `src/config/schedulerUrl.js`, removing localhost fallback behavior from production booking/deep links.
- **CI/build guard for localhost scheduler leaks:** Updated `Dockerfile` to default `VITE_CALCOM_URL` to `https://scheduler.aishacrm.com` and fail build if compiled `dist` assets contain `localhost:3002`. Updated `.github/workflows/docker-release.yml` to require and pass `VITE_CALCOM_URL` at build time.
