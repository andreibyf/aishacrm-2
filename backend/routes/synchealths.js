...
export default function createSyncHealthRoutes(_pgPool) {
  ...
  router.post('/', invalidateCache('synchealths'), async (req, res) => {
    try {
  ...
  router.put('/:id', invalidateCache('synchealths'), async (req, res) => {
    try {
      const { id } = req.params;
  ...
  router.delete('/:id', invalidateCache('synchealths'), async (req, res) => {
    try {
      const { id } = req.params;
      const { getSupabaseClient } = await import('../lib/supabase-db.js');
      const supabase = getSupabaseClient();
