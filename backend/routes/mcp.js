...
export default function createMCPRoutes(_pgPool) {
  ...
  router.post('/market-insights', async (req, res) => {
    ...
    try {
      ...
      // Synthesis lives in backend/lib/marketInsights/synthesize.js so the growth insight
      // runner can produce the SAME rich report in one unified run.
      const result = await synthesizeMarketInsights({ supabase: supa, tenantId, body });
