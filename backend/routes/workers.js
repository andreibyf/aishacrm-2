...
export default function createWorkersRoutes(_pgPool) {
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      let q = supabase.from('workers').select('*').eq('id', req.idScope.id);
  ...
  router.put('/:id', invalidateCache('workers'), async (req, res) => {
    try {
      ...
      if (emergency_contact_name !== undefined)
        payload.emergency_contact_name = toNullableString(emergency_contact_name);
      if (emergency_contact_phone !== undefined)
        payload.emergency_contact_phone = toNullableString(emergency_contact_phone);
