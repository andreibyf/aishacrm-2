...
// ---------------------------------------------------------------------------
// Tenant slug validation (mirror of tenant_slug_format CHECK from migration 161)
// ---------------------------------------------------------------------------

/** Mirrors the SQL CHECK constraint: lowercase, digits, hyphens between
 *  alphanumerics, length 2..64. Used by PUT /:id and POST / before write so
 *  we can return a friendly 400 instead of a 23514 constraint violation. */
...
/** Best-effort slugifier matching migration 161's backfill SQL. Used by the
 *  CREATE path so callers can omit slug and get a sensible default. */
export function slugifyTenantName(name, fallbackId) {
  const base = (name || '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 64);
  if (base.length >= 2 && TENANT_SLUG_PATTERN.test(base)) return base;
  return `tenant-${(fallbackId || '').slice(0, 8) || 'unknown'}`;
}
 ...
 * All modules in this list are ENABLED by default.
 */
...
/**
 * Default modules that get a seeded row but stay DISABLED for new tenants.
 *
 * Finance Ops is a controlled-rollout module: every new tenant should get a
 * modulesettings row (so the Finance v2 gate has something to read and the
 * module is visible/toggleable in Module Settings), but it must default OFF so
 * the per-tenant gate stays meaningful — an admin/superadmin enables it
 * explicitly per tenant. The key is the canonical 'financeOps' the backend
 * gate matches (backend/lib/finance/financeModuleGate.js). The process-level
 * ENABLE_FINANCE_OPS env remains a separate master switch that mounts the
 * route at all; this row is the second, per-tenant gate.
 */
 ...
 * @returns {Array<{tenant_id: string, module_name: string, settings: object, is_enabled: boolean}>}
 */
export function buildDefaultModuleRows(tenantId) {
  return [
    ...
    ...DEFAULT_DISABLED_MODULES.map((moduleName) => ({
      tenant_id: tenantId,
      module_name: moduleName,
      settings: {},
      is_enabled: false,
...
async function initializeModuleSettingsForTenant(supabase, tenantId) {
  try {
    const moduleRows = buildDefaultModuleRows(tenantId);
...
export default function createTenantRoutes(_pgPool) {
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      // Resolve slug: explicit value (validated + uniqueness-checked) or
      // auto-derived from name. NOT NULL on the column means every insert
      // must produce one. Collisions get a `-N` suffix until we find a
      // free slot (capped at 50 attempts, defensive — pathological case).
      let resolvedSlug;
      if (requestedSlug !== undefined && requestedSlug !== null && requestedSlug !== '') {
        if (!isValidTenantSlug(requestedSlug)) {
          return res.status(400).json({
            status: 'error',
            message:
              'Invalid slug. Use 2–64 lowercase letters, digits, and hyphens; must start and end with an alphanumeric (no leading/trailing or doubled hyphen).',
          });
        }
        ...
        if (clash && clash.length > 0) {
          return res.status(409).json({
            status: 'error',
            message: 'Slug already in use by another tenant.',
          });
        }
        ...
        for (let attempt = 1; attempt <= 50; attempt++) {
          ...
          candidate = `${baseSlug.slice(0, 60)}-${attempt + 1}`;
        }
        if (!resolvedSlug) {
          return res.status(409).json({
            status: 'error',
            message: 'Could not derive a unique slug from the tenant name. Provide one explicitly.',
          });
        }
      }
  ...
  router.put('/:id', async (req, res) => {
    try {
      ...
      // Validate slug shape & uniqueness BEFORE writing — surfaces a 400
      // with a meaningful message instead of a generic 23505/23514 from the
      // tenant_slug_unique / tenant_slug_format CHECK constraint.
      if (slug !== undefined) {
        ...
        const supabaseChk = getSupabaseAdmin();
        const slugChk = supabaseChk
          .from('tenant')
          .select('id')
          .eq('slug', slug)
          .neq(isUUID ? 'id' : 'tenant_id', id)
          .limit(1);
        const { data: clash } = await slugChk;
        if (clash && clash.length > 0) {
          return res.status(409).json({
            status: 'error',
            message: 'Slug already in use by another tenant.',
