...
export default function createTestingRoutes(_pgPool) {
        ...
        data: {
          suites: [
            ...
            { id: 'api', name: 'API Integration', description: 'Test external API integrations' },
            {
              ...
              description: 'Test input validation and constraints',
            },
            {
              id: 'permissions',
              name: 'Permissions',
            ...
            },
            {
              ...
              description: 'Test response times and load handling',
            },
            {
              id: 'database',
  ...
  router.post('/trigger-e2e', async (req, res) => {
    try {
      ...
      const GITHUB_WORKFLOW_FILE =
        process.env.GITHUB_WORKFLOW_FILE || process.env.WORKFLOW_FILE || 'e2e.yml';
  ...
  router.get('/workflow-status', async (req, res) => {
    try {
      ...
      const GITHUB_TOKEN = process.env.GITHUB_TOKEN || process.env.GH_TOKEN;
      const GITHUB_REPO_OWNER =
        process.env.GITHUB_REPO_OWNER || process.env.GITHUB_REPOSITORY_OWNER || 'andreibyf';
      const GITHUB_REPO_NAME =
        process.env.GITHUB_REPO_NAME || process.env.GITHUB_REPOSITORY_NAME || 'aishacrm-2';
      ...
      const createdAfter = req.query.created_after
        ? new Date(String(req.query.created_after))
        : null;
      ...
      if (!ghRes.ok) {
        ...
        return res.status(ghRes.status).json({
          status: 'error',
          message: `GitHub runs fetch failed: ${ghRes.status} ${ghRes.statusText}`,
          details: text,
        });
      }
    ...
    try {
      ...
      try {
        ...
        if (!validation.valid) {
          ...
          const probeResp = await fetch(validation.url.toString(), { method: 'GET' });
      ...
      const endpoints = [
        ...
        { method: 'GET', path: `/api/accounts?tenant_id={TENANT_ID}` },
        {
          method: 'POST',
          path: '/api/accounts',
          body: { tenant_id: tenantId, name: `Scan Account ${Date.now()}` },
        },
        ...
        {
          method: 'POST',
          path: '/api/telephony/test-webhook',
      ...
      for (let i = 0; i < endpoints.length; i += BATCH_SIZE) {
        ...
        const batchPromises = batch.map(async (ep) => {
          ...
          try {
            ...
            const resp = await fetch(validation.url.toString(), fetchOpts);
    ...
    at next (express/lib/router/route.js:137:13)`;
      ...
      if (create_github_issue) {
        try {
          ...
          const GITHUB_REPO_OWNER =
            process.env.REPO_OWNER || process.env.GITHUB_REPO_OWNER || 'andreibyf';
          const GITHUB_REPO_NAME =
            process.env.REPO_NAME || process.env.GITHUB_REPO_NAME || 'aishacrm-2';
...
_Auto-generated by AiSHA CRM Health Monitoring System_`;
  ...
  // GET /api/testing/playbook-seeding - Verify industry playbook resolver (no DB writes)
  // Returns which playbooks would be seeded for given industry strings.
  // Safe to call in production – read-only.
  // Query param: industry (optional, defaults to testing a standard set of variants)
  router.get('/playbook-seeding', async (req, res) => {
    try {
      const { getIndustryPlaybookTemplates } = await import('./tenants.js');

      const industry = req.query.industry;

      if (industry) {
        // Single-industry lookup
        ...
        return res.json({
          ...
          data: {
            ...
            playbooks: templates.map((p) => p.name || p.trigger_type || '(unnamed)'),
          },
        });
      }

      // Default: test a canonical set of real-estate variants
      const variants = [
      ...
      const results = variants.map((v) => {
        const templates = getIndustryPlaybookTemplates(v);
        return { industry: v, count: templates.length, ok: templates.length > 0 };
      });
      ...
      return res.json({
        ...
        data: { results, allPass },
        summary: allPass
          ? `All ${variants.length} real-estate industry variants resolve correctly`
          : `Some variants returned 0 playbooks — seeding fix may not be deployed`,
      });
    } catch (error) {
      res.status(500).json({ status: 'error', message: error.message });
    }
