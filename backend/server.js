  ...
  helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        'frame-ancestors': [
          "'self'",
          ...(process.env.ALLOWED_DOCS_ORIGINS?.split(',')
            .map((s) => s.trim())
            .filter(Boolean) || [
            'http://localhost:5173',
...
const HEARTBEAT_INTERVAL_MS = Math.max(
  15000,
  parseInt(process.env.HEARTBEAT_INTERVAL_MS || '60000', 10),
);
...
if (process.env.NODE_ENV === 'production') {
  ...
  let parsedSchedulerUrl = null;
  try {
    parsedSchedulerUrl = schedulerUrl ? new URL(schedulerUrl) : null;
  } catch {
    parsedSchedulerUrl = null;
  }

  const invalidSchedulerUrl =
    !parsedSchedulerUrl ||
    parsedSchedulerUrl.protocol !== 'https:' ||
    !parsedSchedulerUrl.origin ||
    Boolean(parsedSchedulerUrl.username) ||
    Boolean(parsedSchedulerUrl.password);

  if (invalidSchedulerUrl) {
    logger.fatal(
...
╚═══════════════════════════════════════════════════════════╝
  ...
  ensureStorageBucketExists().catch((err) => logger.error({ err }, 'Bucket ensure failed'));
