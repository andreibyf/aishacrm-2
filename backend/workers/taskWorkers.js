...
export function startTaskWorkers() {
    ...
    try {
      ...
      if (!force_assignee) {
        ...
        if (desc.match(/\b(opportunity|won|lost|deal|close|proposal|quote|negotiat|pipeline)\b/)) {
          ...
          desc.match(
            /\b(schedule|appointment|meeting|calendar|set up a call|book|reminder|remind)\b/,
          )
        ) {
      ...
      // 2b. Emit task_enqueued first so inbox panel populates
      emitTaskEnqueued({
        run_id,
        trace_id: run_id,
        span_id: randomUUID(),
  ...
  taskQueue.process('execute-task', 1, async (job) => {
    ...
    try {
      ...
      logger.info(
        `[ExecuteTask] Using agent profile: ${agentProfile.display_name} (${agentProfile.model})`,
      );
      ...
      if (entity_type && entity_id) {
        ...
        if (hasProfileData || hasRelatedData) {
          ...
          if (hasProfileData) {
            ...
            if (profile.account_id && entity_type !== 'account')
              contextInfo += `  ACCOUNT_ID: ${profile.account_id}\n`;
            if (profile.contact_id && entity_type !== 'contact')
              contextInfo += `  CONTACT_ID: ${profile.contact_id}\n`;
            if (profile.lead_id && entity_type !== 'lead')
              contextInfo += `  LEAD_ID: ${profile.lead_id}\n`;
          ...
          try {
            ...
            if (tableName) {
              ...
              if (entityData) {
                ...
                if (entityData.account_id && entity_type !== 'account')
                  contextInfo += `  ACCOUNT_ID: ${entityData.account_id}\n`;
                if (entityData.contact_id && entity_type !== 'contact')
                  contextInfo += `  CONTACT_ID: ${entityData.contact_id}\n`;
                if (entityData.lead_id && entity_type !== 'lead')
                  contextInfo += `  LEAD_ID: ${entityData.lead_id}\n`;
