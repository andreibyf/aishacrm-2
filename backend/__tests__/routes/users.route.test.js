...
describe('Users Routes', { skip: !SHOULD_RUN }, () => {
  ...
  test('GET /api/users/:id returns 404 for non-existent', async () => {
    const res = await fetch(`${BASE_URL}/api/users/${NONEXISTENT_ID}?tenant_id=${TENANT_ID}`);
  ...
  test('POST /api/users requires email', async () => {
    ...
    assert.ok(
      ...
      `expected 400 or 422 for missing email, got ${res.status}`,
    );
  });

  test('PUT /api/users/:id validates user exists', async () => {
    const res = await fetch(`${BASE_URL}/api/users/${NONEXISTENT_ID}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
