...
describe('Accounts Routes', { skip: !SHOULD_RUN }, () => {
  before(async () => {
    // Seed test accounts using TestFactory
    const accountA = TestFactory.account({
    ...
    const a = await createAccount(accountA);
    ...
    const accountB = TestFactory.account({
    ...
    const b = await createAccount(accountB);
    // Accept 200 or 201 - API currently returns 200 for creation
  ...
  test('GET /api/accounts returns 200 with tenant_id', async () => {
    ...
    assert.ok(
      json.data?.accounts || Array.isArray(json.data),
      'expected accounts array in response',
    );
  ...
  test('DELETE /api/accounts/:id removes account', async () => {
    // Create a temporary account to delete using TestFactory
    const tempAccount = TestFactory.account({
      name: 'Temp Delete Account',
      account_type: 'prospect',
      tenant_id: TENANT_ID,
    });

    const temp = await createAccount(tempAccount);
  ...
  test('GET /api/accounts/search returns matching accounts', async () => {
    ...
    assert.ok(
      json.data?.accounts || Array.isArray(json.data),
      'expected accounts array in search response',
    );
