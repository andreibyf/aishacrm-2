...
before(async () => {
  ...
  // Seed test opportunities using TestFactory
  ...
  const a = await createOpportunity(oppA);
  assert.ok(
    [200, 201].includes(a.status),
    `create opportunity A failed: ${JSON.stringify(a.json)}`,
  );
  ...
  const b = await createOpportunity(oppB);
  assert.ok(
    [200, 201].includes(b.status),
    `create opportunity B failed: ${JSON.stringify(b.json)}`,
  );
...
(SHOULD_RUN ? test : test.skip)('GET /api/opportunities returns 200 with tenant_id', async () => {
  ...
  assert.ok(
    json.data?.opportunities || Array.isArray(json.data),
    'expected opportunities array in response',
  );
...
(SHOULD_RUN ? test : test.skip)('DELETE /api/opportunities/:id removes opportunity', async () => {
  // Create a temporary opportunity to delete using TestFactory
  const tempOpp = TestFactory.opportunity({
  ...
  const temp = await createOpportunity(tempOpp);
...
(SHOULD_RUN ? test : test.skip)(
  'GET /api/opportunities/search returns matching opportunities',
  async () => {
    ...
    assert.ok(
      json.data?.opportunities || Array.isArray(json.data),
      'expected opportunities array in search response',
    );
