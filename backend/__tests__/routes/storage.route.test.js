...
(SHOULD_RUN ? test : test.skip)('POST /api/storage/artifacts requires payload parameter', async () => {
  const res = await fetch(`${BASE_URL}/api/storage/artifacts`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
(SHOULD_RUN ? test : test.skip)('POST /api/storage/artifacts stores and retrieves artifact (if R2 configured)', async () => {
  ...
  assert.ok([201, 500].includes(createRes.status), `expected 201 or 500, got ${createRes.status}`);
  
  if (createRes.status === 500) {
    console.log('R2 artifact creation failed (likely missing migration) - skipping retrieval test');
    return;
  }
  ...
  const getRes = await fetch(`${BASE_URL}/api/storage/artifacts/${artifactId}?tenant_id=${TENANT_ID}`, {
    headers: { 'x-tenant-id': TENANT_ID }
  });
...
(SHOULD_RUN ? test : test.skip)('GET /api/storage/artifacts/:id enforces tenant isolation', async () => {
