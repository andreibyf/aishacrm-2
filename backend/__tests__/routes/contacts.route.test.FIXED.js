...
before(async () => {
  ...
  // Seed test contacts using TestFactory
  ...
  const contactB = TestFactory.contact({
    first_name: 'Unit',
  ...
  const b = await createContact(contactB);
...
(SHOULD_RUN ? test : test.skip)('DELETE /api/contacts/:id removes contact', async () => {
  // Create a temporary contact to delete using TestFactory
  const tempContact = TestFactory.contact({
    first_name: 'Temp',
    last_name: 'DeleteMe',
    tenant_id: TENANT_ID,
  });

  const temp = await createContact(tempContact);
...
(SHOULD_RUN ? test : test.skip)('GET /api/contacts/search returns matching contacts', async () => {
  ...
  assert.equal(json.status, 'success');
  assert.ok(
    json.data?.contacts || Array.isArray(json.data),
    'expected contacts array in search response',
  );
