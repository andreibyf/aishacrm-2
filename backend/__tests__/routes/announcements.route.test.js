...
describe('Announcements Routes', { skip: !SHOULD_RUN }, () => {
  after(async () => {
    ...
    for (const id of createdIds) {
      try {
        await fetch(`${BASE_URL}/api/announcements/${id}?tenant_id=${TENANT_ID}`, {
          method: 'DELETE',
  ...
  test('GET /api/announcements returns announcements list', async () => {
    ...
    if (res.status === 200) {
      ...
      assert.ok(
        Array.isArray(json) || json.data || json.announcements,
        'expected announcements data',
      );
    }
  });
  ...
  test('POST /api/announcements without required fields returns error', async () => {
    ...
    assert.ok(
      ...
      `expected error for missing data, got ${res.status}`,
    );
  });
  ...
  test('POST /api/announcements with valid data creates announcement', async () => {
    ...
    assert.ok(
      [200, 201, 401, 422].includes(res.status),
      `expected creation response, got ${res.status}`,
    );
