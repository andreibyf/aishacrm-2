...
describe('AI Execution Record Telemetry', { skip: !SHOULD_RUN }, () => {
  ...
  test('POST /api/ai/chat emits execution_record on success', async () => {
    ...
    assert.ok(
      ...
      `expected 200, 400, 401 or 500, got ${res.status}`,
    );
    ...
    if (res.status === 200) {
      ...
      assert.equal(
        execRecord.nodeId,
        'ai:chat:execution_record',
        'nodeId should be ai:chat:execution_record',
      );
      ...
      assert.ok(
        Array.isArray(execRecord.toolsCalled) || execRecord.toolsCalled === null,
  ...
  test('execution_record includes taskId and requestId', async () => {
    ...
    if (res.status === 200) {
      ...
      if (execRecord) {
        ...
        assert.ok(execRecord.requestId, 'requestId should be present');
        assert.ok(
          execRecord.requestId.startsWith('req_'),
  ...
  test('execution_record captures intent when classified', async () => {
    ...
    if (res.status === 200) {
      ...
      if (execRecord) {
        ...
        assert.ok(
          typeof execRecord.intent === 'string' || execRecord.intent === null,
