...
before(async () => {
  ...
  // Seed two leads with different statuses using TestFactory
  ...
  const b = await createLead(leadB);
...
(SHOULD_RUN ? test : test.skip)('GET /api/leads/search returns matching leads', async () => {
  ...
  assert.equal(json.status, 'success');
  assert.ok(
    json.data?.leads || Array.isArray(json.data),
    'expected leads array in search response',
  );
