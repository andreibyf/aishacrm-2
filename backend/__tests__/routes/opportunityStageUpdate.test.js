...
import { TENANT_ID as TEST_TENANT } from '../testConstants.js';
...
(SHOULD_RUN ? test : test.skip)(
  'Stage update from prospecting to qualification persists',
  async () => {
    ...
    const { res: fetchRes, body: fetchBody } = await jsonFetch(
      `/api/v2/opportunities/${createdOppId}?tenant_id=${TEST_TENANT}`,
    );
...
(SHOULD_RUN ? test : test.skip)(
  'Stage update from qualification to proposal persists',
  async () => {
    assert.ok(createdOppId, 'Test opportunity not created');
