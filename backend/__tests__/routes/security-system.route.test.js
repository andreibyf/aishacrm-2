...
describe('Security Routes', { skip: !SHOULD_RUN }, () => {
  ...
  test('DELETE /api/apikeys/:id validates key exists', async () => {
    const res = await fetch(`${BASE_URL}/api/apikeys/${NONEXISTENT_ID}?tenant_id=${TENANT_ID}`, {
      method: 'DELETE',
    });
    assert.ok(
      [400, 404, 401, 403].includes(res.status),
      `expected 400/404/401/403, got ${res.status}`,
    );
  });
...
describe('System Routes', { skip: !SHOULD_RUN }, () => {
  test('GET /api/system/health returns health status', async () => {
    ...
    const json = await res.json();
    assert.ok(
      json.status === 'success' || json.status === 'ok' || json.healthy,
