...
async function getSuggestions(params = {}) {
  ...
  const res = await fetch(`${BASE_URL}/api/ai/suggestions?${query}`, { headers: getAuthHeaders() });
...
async function cleanup() {
  ...
  for (const id of createdSuggestions) {
    try {
      await fetch(`${BASE_URL}/api/ai/suggestions/${id}`, {
        method: 'DELETE',
  ...
  for (const id of createdOpportunities) {
    try {
      await fetch(`${BASE_URL}/api/opportunities/${id}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
    } catch {
...
describe('AI Triggers Worker', { skip: !SHOULD_RUN }, () => {
  ...
  test('Worker exports startAiTriggersWorker and stopAiTriggersWorker', async () => {
    const { startAiTriggersWorker, stopAiTriggersWorker } = await import(
      '../../lib/aiTriggersWorker.js'
    );

    assert.equal(
      typeof startAiTriggersWorker,
      'function',
      'startAiTriggersWorker should be a function',
    );
    assert.equal(
      typeof stopAiTriggersWorker,
      'function',
      'stopAiTriggersWorker should be a function',
    );
  });
  ...
  test('Worker exports getPendingSuggestions', async () => {
    ...
    assert.equal(
      ...
      'function',
      'getPendingSuggestions should be a function',
    );
  });
});
  ...
  test('GET /api/ai/suggestions supports trigger_id filter', async () => {
    ...
    for (const s of json.data?.suggestions || []) {
      assert.equal(
        s.trigger_id,
        'lead_stagnant',
        'All suggestions should have lead_stagnant trigger_id',
      );
    }
  });
