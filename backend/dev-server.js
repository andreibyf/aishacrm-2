...
function checkRestartLimit() {
  ...
  restartTimestamps = restartTimestamps.filter((ts) =>
    now - ts < RESTART_WINDOW_MS
  );

  if (restartTimestamps.length >= MAX_RESTARTS_PER_MINUTE) {
    log(
      `❌ Restart limit exceeded (${MAX_RESTARTS_PER_MINUTE} restarts in 1 minute)`,
      colors.red,
    );
    log(
      "This usually indicates a crash loop or configuration issue.",
      colors.yellow,
    );
  ...
  if (timeSinceLastRestart < RESTART_COOLDOWN_MS) {
    log(
      `⏳ Restart cooldown active (${
        Math.ceil((RESTART_COOLDOWN_MS - timeSinceLastRestart) / 1000)
      }s remaining)`,
      colors.yellow,
    );
    return;
...
log(
  `📋 Restart policy: Max ${MAX_RESTARTS_PER_MINUTE} restarts per minute, ${
