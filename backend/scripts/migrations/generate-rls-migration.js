...
const policyRegex =
  /CREATE\s+POLICY\s+"([^"]+)"\s+ON\s+"public"\."([^"]+)"(.*?)\s+USING\s+\((.+?)\)(?:\s+WITH\s+CHECK\s+\((.+?)\))?;/gis;
...
while ((match = policyRegex.exec(content)) !== null) {
  ...
  if (usingClause.includes('tenant_id_text') || usingClause.includes('tenant_id_legacy')) {
    ...
    const toMatch = middlePart.match(
      /TO\s+((?:(?:"[^"]+")|(?:\w+))(?:\s*,\s*(?:(?:"[^"]+")|(?:\w+)))*)/i,
    );
...
for (const [tableName, policies] of Object.entries(policiesByTable).sort()) {
  console.log(
    `   ${tableName.padEnd(30)} ${policies.length} ${policies.length === 1 ? 'policy' : 'policies'}`,
  );
}
