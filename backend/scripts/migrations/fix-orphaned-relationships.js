  ...
  for (const note of orphanedNotes || []) {
    ...
    if (note.metadata?.source_activity_id) {
      ...
      if (activity?.related_to && activity?.related_id) {
        console.log(
          `  Note ${note.id}: Linking via source_activity → ${activity.related_to}:${activity.related_id}`,
        );
...
async function fixActivitiesMissingDenormalizedFields() {
  ...
  console.log(
    `Found ${activities?.length || 0} activities with related_id but missing related_name`,
  );
...
async function fixOpportunitiesMissingContactName() {
  ...
  console.log(
    `Found ${opportunities?.length || 0} opportunities with contact_id but missing contact_name`,
  );
...
async function main() {
  try {
    ...
    console.log('='.repeat(60));
    console.log(
      `Notes:         ${stats.notes.checked} checked, ${stats.notes.fixed} ${dryRun ? 'would be' : ''} fixed, ${stats.notes.errors} errors`,
    );
    console.log(
      `Activities:    ${stats.activities.checked} checked, ${stats.activities.fixed} ${dryRun ? 'would be' : ''} fixed, ${stats.activities.errors} errors`,
    );
    console.log(
      `Opportunities: ${stats.opportunities.checked} checked, ${stats.opportunities.fixed} ${dryRun ? 'would be' : ''} fixed, ${stats.opportunities.errors} errors`,
    );
