...
async function main() {
  ...
  try {
    ...
    const sslConfig =
      connectionString.includes('sslmode=require') || connectionString.includes('supabase')
    ...
    console.error(
      '2. For Supabase, ensure DATABASE_URL uses Transaction pooler (not Session pooler)',
    );
    console.error('3. Try setting NODE_TLS_REJECT_UNAUTHORIZED=0 temporarily:');
    console.error(
      '   doppler run -- env NODE_TLS_REJECT_UNAUTHORIZED=0 node apply-single-sql.js ...',
    );
    process.exit(1);
  } finally {
    try {
