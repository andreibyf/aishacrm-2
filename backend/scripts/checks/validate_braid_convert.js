...
function errStr(res) {
  return res?.error?.message || JSON.stringify(res?.error || res);
}
...
async function run() {
  ...
  // 1) Create Lead
  {
    ...
    if (ok(res) && res.value?.id) {
      leadId = res.value.id;
      logStep('create_lead', 'PASS', `id=${leadId}`);
    } else {
      logStep('create_lead', 'FAIL', errStr(res));
      process.exit(1);
    }
  }
  ...
  {
    const res = await executeBraidTool(
      'delete_contact',
      { tenant: tenantSlug, contact_id: contactId },
      tenantRecord,
      null,
    );
  ...
  {
    // Create a fresh lead
    const resLead = await executeBraidTool(
      'create_lead',
      {
        tenant: tenantSlug,
        first_name: 'Conv2',
    ...
    if (!(ok(resLead) && resLead.value?.id)) {
      logStep('create_lead (flow2)', 'FAIL', errStr(resLead));
      process.exit(1);
    }
    ...
    const resConv = await executeBraidTool(
      'convert_lead_to_account',
      convertArgs,
      tenantRecord,
      null,
    );
    if (!(ok(resConv) && resConv.value?.contact?.id)) {
      logStep('convert (flow2)', 'FAIL', errStr(resConv));
      process.exit(1);
    }
    ...
    if (!account2Id) {
      logStep('convert (flow2.account)', 'FAIL', 'no account returned');
      process.exit(1);
    }
    if (!opportunity2Id) {
      logStep('convert (flow2.opportunity)', 'FAIL', 'no opportunity returned');
      process.exit(1);
    }
    logStep(
      'convert (flow2)',
      'PASS',
      `contact=${contact2Id}, account=${account2Id}, opp=${opportunity2Id}`,
    );
    ...
    try {
      if (supabasePool) {
        ...
        if (trans.rows.length === 0) {
          logStep('transition_check (flow2)', 'FAIL', 'no transition record');
          process.exit(1);
        } else {
          logStep('transition_check (flow2)', 'PASS');
        }
      }
    } catch (e) {
      const _ignored2 = e;
    ...
    const delOpp = await executeBraidTool(
      'delete_opportunity',
      { tenant: tenantSlug, opportunity_id: opportunity2Id },
      tenantRecord,
      null,
    );
    if (!(ok(delOpp) && delOpp.value === true)) {
      logStep('delete_opportunity (flow2)', 'FAIL', errStr(delOpp));
      process.exit(1);
    }
    logStep('delete_opportunity (flow2)', 'PASS');
    const delContact = await executeBraidTool(
      'delete_contact',
      { tenant: tenantSlug, contact_id: contact2Id },
      tenantRecord,
      null,
    );
    if (!(ok(delContact) && delContact.value === true)) {
      logStep('delete_contact (flow2)', 'FAIL', errStr(delContact));
      process.exit(1);
    }
    logStep('delete_contact (flow2)', 'PASS');
    const delAccount = await executeBraidTool(
      'delete_account',
      { tenant: tenantSlug, account_id: account2Id },
      tenantRecord,
      null,
    );
    if (!(ok(delAccount) && delAccount.value === true)) {
      logStep('delete_account (flow2)', 'FAIL', errStr(delAccount));
      process.exit(1);
    }
  ...
  // 5) Convert into an existing Account
  {
    ...
    const accRes = await executeBraidTool(
      'create_account',
      {
        tenant: tenantSlug,
    ...
    if (!(ok(accRes) && accRes.value?.id)) {
      logStep('create_account (flow3)', 'FAIL', errStr(accRes));
      process.exit(1);
    }
    ...
    // Create lead
    const resLead3 = await executeBraidTool(
      'create_lead',
      {
        tenant: tenantSlug,
        first_name: 'Conv3',
    ...
    if (!(ok(resLead3) && resLead3.value?.id)) {
      logStep('create_lead (flow3)', 'FAIL', errStr(resLead3));
      process.exit(1);
    }
    ...
    const resConv3 = await executeBraidTool(
      'convert_lead_to_account',
      {
        tenant: tenantSlug,
        lead_id: lead3Id,
    ...
    if (!(ok(resConv3) && resConv3.value?.contact?.id)) {
      logStep('convert (flow3)', 'FAIL', errStr(resConv3));
      process.exit(1);
    }
    const contact3Id = resConv3.value.contact.id;
    const linkedAccId = resConv3.value.contact.account_id || null;
    if (linkedAccId !== existingAccId) {
      logStep(
        'convert (flow3.account_link)',
        'FAIL',
        `expected ${existingAccId}, got ${linkedAccId}`,
      );
      process.exit(1);
    }
    ...
    try {
      if (supabasePool) {
        ...
        if (trans.rows.length === 0) {
          logStep('transition_check (flow3)', 'FAIL', 'no transition record');
          process.exit(1);
        } else {
          logStep('transition_check (flow3)', 'PASS');
        }
      }
    } catch (e) {
      const _ignored3 = e;
    ...
    const delContact3 = await executeBraidTool(
      'delete_contact',
      { tenant: tenantSlug, contact_id: contact3Id },
      tenantRecord,
      null,
    );
    if (!(ok(delContact3) && delContact3.value === true)) {
      logStep('delete_contact (flow3)', 'FAIL', errStr(delContact3));
      process.exit(1);
    }
    logStep('delete_contact (flow3)', 'PASS');
    const delAccount3 = await executeBraidTool(
      'delete_account',
      { tenant: tenantSlug, account_id: existingAccId },
      tenantRecord,
      null,
    );
    if (!(ok(delAccount3) && delAccount3.value === true)) {
      logStep('delete_account (flow3)', 'FAIL', errStr(delAccount3));
      process.exit(1);
    }
...
run().catch((e) => {
  console.error('Unexpected error during lead conversion validation:', e);
  process.exit(1);
});
