...
async function run() {
  ...
  // 2) Create Note related to the Account
  {
    ...
    if (ok(res) && res.value?.note?.id) {
      noteId = res.value.note.id;
      logStep('create_note', 'PASS', `id=${noteId}`);
    } else {
      logStep('create_note', 'FAIL', errStr(res));
  ...
  // 3) Update Note content
  {
    ...
    if (ok(res) && res.value?.note?.content === 'Updated note content.') {
      logStep('update_note', 'PASS');
    } else {
      logStep('update_note', 'FAIL', errStr(res));
  ...
  // 4) Get notes for record and ensure our note is present
  {
    ...
    if (
      ok(res) &&
      Array.isArray(res.value?.notes) &&
      res.value.notes.some((n) => n.id === noteId)
    ) {
  ...
  // 5) Delete Note
  {
    const res = await executeBraidTool(
      'delete_note',
      { tenant: tenantSlug, note_id: noteId },
      tenantRecord,
      null,
    );
    if (ok(res) && res.value === true) {
      logStep('delete_note', 'PASS');
    } else {
      logStep('delete_note', 'FAIL', errStr(res));
...
run().catch((e) => {
  console.error('Unexpected error during notes CRUD validation:', e);
  process.exit(1);
});
