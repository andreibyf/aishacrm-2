...
const toolsToTest = [
  ...
  // Notes
  {
    ...
    args: { tenant: tenantSlug, related_to_type: 'account', related_to_id: '' },
  },

  // Opportunities
  {
    name: 'list_opportunities_by_stage',
    args: { tenant: tenantSlug, stage: 'prospecting', limit: 10 },
  },
  ...
  for (const { name, args } of toolsToTest) {
    ...
    try {
      ...
      if (result?.tag === 'Ok') {
        ...
        const arrayCount = Array.isArray(value)
          ? value.length
          : keys.some((k) => Array.isArray(value[k]))
            ? keys
                .filter((k) => Array.isArray(value[k]))
                .map((k) => `${k}:${value[k].length}`)
                .join(', ')
            : 'N/A';

        console.log(
          `✅ OK (${Array.isArray(value) ? value.length + ' items' : typeof value === 'object' ? keys.length + ' keys' : 'scalar'})`,
        );
