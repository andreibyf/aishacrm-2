...
# Find all test files
TEST_FILES=$(find __tests__ -name "*.test.js" 2>/dev/null | sort)
...
echo "=========================================="
echo "Running $TOTAL backend test files"
...
for FILE in $TEST_FILES; do
    ...
    START_TIME=$(date +%s)
    timeout "$TIMEOUT_SECONDS" node --test --test-reporter tap "$FILE" 2>&1 > /tmp/test-output.txt
    EXIT_CODE=$?
    END_TIME=$(date +%s)
