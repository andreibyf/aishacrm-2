...
routeFiles.forEach((file) => {
  ...
  if (content.includes("router.put('/:id'") || content.includes('router.put("/:id"')) {
    ...
    if (putMatch) {
      ...
      if (
        putBlock.includes('currentMetadata') ||
        putBlock.includes('updatedMetadata') ||
        putBlock.includes('...metadata')
      ) {
  ...
  if (analysis.hasPutEndpoint && analysis.hasMetadataColumn && !analysis.hasMetadataMerging) {
    analysis.issues.push(
      '⚠️  PUT endpoint has metadata field but NO merging logic - fields may be lost!',
    );
  }

  if (analysis.hasPutEndpoint && analysis.hasMetadataColumn && !analysis.hasExpandFunction) {
    analysis.issues.push(
      '⚠️  Has metadata but NO expand function - frontend may not receive fields!',
    );
  }

  if (
    analysis.hasPutEndpoint &&
    analysis.extractedFields.length > 0 &&
    !analysis.extractedFields.includes('metadata')
  ) {
...
routesWithPut.forEach((r) => {
  ...
  if (r.extractedFields.length > 0) {
    console.log(
      `   Fields: ${r.extractedFields.slice(0, 5).join(', ')}${r.extractedFields.length > 5 ? '...' : ''}`,
    );
  }
...
console.log(
  `Routes properly handling metadata: ${results.filter((r) => r.hasPutEndpoint && r.hasMetadataMerging && r.hasExpandFunction).length}`,
);
