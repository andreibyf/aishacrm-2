...
async function getDatabaseColumns(tableName) {
  try {
    const { data: _data, error } = await supa.from(tableName).select('*').limit(0); // Get schema only
...
function extractFieldsFromFrontend(tableName) {
  ...
  const _patterns = [
    ...
    new RegExp(
      `(?:address_1|address_2|city|state|zip|country|employee_count|tags|display_name|navigation_permissions|phone|email|annual_revenue|description|status|type|website|industry|company|job_title|title|subject|body|due_date|close_date|amount|probability|stage|source|first_name|last_name|mobile|hire_date|employment_status|employment_type|hourly_rate|emergency_contact_name|emergency_contact_phone|skills|manager_employee_id|is_active)`,
      'g',
    ),
  ];
        ...
        if (entry.isDirectory()) {
          ...
          for (const match of matches) {
            const fieldMatch = match.match(
              /(address_1|address_2|city|state|zip|country|employee_count|tags|display_name|navigation_permissions|phone|email|annual_revenue|description|status|type|website|industry|company|job_title|title|subject|body|due_date|close_date|amount|probability|stage|source|first_name|last_name|mobile|hire_date|employment_status|employment_type|hourly_rate|emergency_contact_name|emergency_contact_phone|skills|manager_employee_id|is_active|num_employees)/,
            );
            if (fieldMatch) {
...
for (const table of TABLES) {
  ...
  console.log(
    `    ${frontendFields.slice(0, 20).join(', ')}${frontendFields.length > 20 ? '...' : ''}`,
  );
...
if (results.totalMismatches > 0) {
  ...
  console.log(
    '  Add migrations to create these columns OR update routes to handle them in metadata.\n',
  );
}
