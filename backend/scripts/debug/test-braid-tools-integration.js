...
async function runTests() {
  ...
  await test('search_contacts tool executes without parameter nesting', async () => {
    ...
    if (result.tag === 'Err' && result.error) {
      console.log(
        `   Note: Got error (this is ok if data doesn't exist): ${result.error.message || result.error.type}`,
      );
    ...
    if (result.tag === 'Ok') {
      ...
      assert(
        Array.isArray(result.value) || result.value === null,
  ...
  await test('list_contacts_for_account tool executes correctly', async () => {
    ...
    assert(result, 'Should return a result');
    assert(result.tag === 'Ok' || result.tag === 'Err', 'Result should have tag');
