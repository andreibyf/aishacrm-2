...
async function smokeTest() {
  ...
  if (!authToken && !tenantId) {
    console.log(
      '\n' +
        colors.red +
        'Cannot continue without authentication - using cookie-based auth' +
        colors.reset,
    );
  ...
  try {
    const res = await fetchWithTimeout(
      `${BASE_URL}/api/v2/activities?tenant_id=${tenantId}&limit=5`,
      {
        headers: authHeaders,
      },
    );
  ...
  try {
    const res = await fetchWithTimeout(
      `${BASE_URL}/api/reports/dashboard-bundle?tenant_id=${tenantId}`,
      {
        headers: authHeaders,
      },
    );
