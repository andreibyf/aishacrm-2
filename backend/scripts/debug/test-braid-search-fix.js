...
async function runTests() {
  ...
  await test('searchContacts maps parameters correctly', async () => {
    ...
    assert(
      positional[0] === args.tenant,
      `First param should be tenant UUID, got ${positional[0]}`,
    );
  ...
  await test('Bug regression: args NOT nested in first parameter', async () => {
    ...
    assert(typeof positional[0] === 'string', 'First parameter should be string tenant UUID');
  });
