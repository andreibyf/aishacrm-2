...
async function fetchTenantConfig(tenantId) {
  try {
    ...
    const { data, error } = await supabase
      .from('tenant')
      .select('id, tenant_id, name, business_model, client_type, industry, metadata')
      .eq('id', tenantId)
      .maybeSingle();
    if (error) throw error;
    if (!data) return null;
    // Map tenant_id → slug for backward compat
    return { ...data, slug: data.tenant_id };
  } catch (err) {
 ...
 */
async function fetchStatusCardConfig(tenantId) {
  try {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase
    ...
    // Table might not exist yet or Supabase error - use defaults
 ...
 */
async function fetchModuleSettings(tenantId) {
  try {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase
      .from('modulesettings')
 ...
 */
export async function buildTenantContextDictionary(_pool, tenantIdOrSlug, options = {}) {
  ...
  if (!bypassCache) {
    try {
      ...
      if (cached) {
        ...
        console.log(
          `[TenantContextDictionary] Cache HIT for ${tenantId} (age: ${cached.cacheAge}ms)`,
        );
        return cached;
      }
    } catch (cacheErr) {
  ...
  // Fetch all context components in parallel (all use Supabase internally)
  const [tenantConfig, entityLabels, statusCards, moduleSettings] = await Promise.all([
    fetchTenantConfig(tenantId),
    fetchEntityLabels(null, tenantId),
    fetchStatusCardConfig(tenantId),
    fetchModuleSettings(tenantId),
  ]);
  ...
  for (const [entityKey, labels] of Object.entries(entityLabels)) {
    terminology[entityKey] = {
      ...
      // Check if customized
      isCustomized:
        labels.singular !== getDefaultLabel(entityKey, 'singular') ||
...
function getBusinessModelNotes(businessModel) {
  const notes = {
    ...
    Hybrid:
      'Mixed model. Lead type determined by presence of company data. Support both B2B and B2C flows.',
  };
