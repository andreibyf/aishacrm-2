  ...
  lead_to_opportunity: {
    ...
    steps: [
      ...
      },
      {
        ...
        args: (input, ctx) => ({
          ...
          name:
            input.opportunity_name || `${ctx.convert?.value?.account?.name || 'New'} Opportunity`,
    ...
    rollback: [
      // Rollback opportunity if created
      {
        ...
        args: (ctx) => ({ opportunity_id: ctx.opportunity?.value?.id, confirmed: true }),
  ...
  account_with_contact: {
    ...
    steps: [
      ...
      },
      {
        ...
        required: false,
        condition: (ctx) =>
          ctx.account?.tag === 'Ok' && (input.contact_first_name || input.contact_email),
      },
    ],
    rollback: [
      {
        tool: 'delete_contact',
        args: (ctx) => ({ contact_id: ctx.contact?.value?.id, confirmed: true }),
        condition: (ctx) => ctx.contact?.tag === 'Ok',
      },
      {
        tool: 'delete_account',
        args: (ctx) => ({ account_id: ctx.account?.value?.id, confirmed: true }),
        condition: (ctx) => ctx.account?.tag === 'Ok',
      },
    ],
  },
    ...
    steps: [
      ...
      },
      {
        ...
        args: (input, ctx) => ({
          ...
          due_date:
            input.followup_date || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
...
export async function executeToolChain(chainName, input, tenantRecord, userId, accessToken) {
  console.log('[Braid Chain] Starting chain: %s', chainName, {
  ...
  try {
    ...
    const steps =
      chain.dynamic && chain.generateSteps ? chain.generateSteps(input) : chain.steps || [];
    ...
    for (const [index, step] of steps.entries()) {
      ...
      let stepArgs;
      try {
        stepArgs = typeof step.args === 'function' ? step.args(input, context) : step.args;
      } catch (argError) {
    ...
    console.error('[Braid Chain] Chain %s threw exception:', chainName, error);
...
async function executeRollback(chain, context, tenantRecord, userId, accessToken) {
  ...
  for (const [index, rollbackStep] of chain.rollback.reverse().entries()) {
    try {
      ...
      const rollbackArgs =
        typeof rollbackStep.args === 'function' ? rollbackStep.args(context) : rollbackStep.args;
      ...
      if (rollbackResult.tag === 'Err') {
        console.error(
          `[Braid Chain] Rollback step ${rollbackStep.tool} failed:`,
      ...
      console.error(
        `[Braid Chain] Rollback step ${rollbackStep.tool} threw exception:`,
        rollbackError,
      );
    }
  }
...
export function validateChain(chainName, input) {
  ...
  // Get steps (may be dynamic)
  const steps = chain.dynamic && chain.generateSteps ? chain.generateSteps(input) : chain.steps;
