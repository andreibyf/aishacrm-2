...
- Exclude: API keys, tokens, passwords, sensitive data`;
    ...
    // Generate summary using AI — defaults to local AI server (free).
    // Override via CONV_SUMMARY_LLM_PROVIDER / CONV_SUMMARY_LLM_MODEL env vars.
    const summaryProvider = process.env.CONV_SUMMARY_LLM_PROVIDER || 'local';
    const summaryModel =
      process.env.CONV_SUMMARY_LLM_MODEL || process.env.LOCAL_LLM_MODEL || 'qwen-14b';
    ...
    const { error: upsertErr } = await supabase.from('ai_conversation_summaries').upsert(
      {
        tenant_id: tenantId,
