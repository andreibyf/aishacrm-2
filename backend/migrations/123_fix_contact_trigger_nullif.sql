...
BEGIN
  ...
  INSERT INTO public.person_profile AS pp (
    ...
    -- FIX: Cast empty string to TEXT to match mobile column type
    pg_catalog.coalesce(pg_catalog.nullif(NEW.mobile, ''::TEXT), NEW.phone), 
    NEW.job_title, NEW.status, NEW.account_id, NEW.account_name, pg_catalog.now()
  )
