...
---
# 11. SUPABASE TRIGGER QUERY POLICY

For all Phase 3 Trigger Engine work, you MUST follow these rules for Supabase access:

- Do NOT use complex raw SQL through the Supabase HTTP/adapter layer:
  - no INTERVAL
  - no NOT EXISTS
  - no nested subqueries
  - no complex JOINs with window functions
- Prefer the Supabase JS client with simple filters:
  - .select, .eq, .lte, .gte, .in, .is, .order, .range
  - multiple small queries composed in JavaScript
- For patterns like:
  - "find candidates" AND
  - "exclude records that already have suggestions",
  do this in TWO simple queries and filter in JavaScript:
  1) fetch candidate rows (e.g., leads, activities) with simple filters
  2) fetch ai_suggestions for that tenant + trigger_id
  3) filter out record_ids already present in suggestions
- Only when there is a clear performance bottleneck should you:
  - move complex logic into a Postgres function, and
  - call it via supabase.rpc('function_name', { ...args })
- NEVER open a direct Postgres connection from this code path, even if technically possible.

All Trigger Engine code must respect this policy.
