...
ENV VITE_CALCOM_URL=$VITE_CALCOM_URL
...
RUN NODE_OPTIONS=--max-old-space-size=512 npm ci --include=dev --legacy-peer-deps

# Copy source code
COPY . .

# Ensure clean build (remove any residual dist folder)
RUN rm -rf dist/
