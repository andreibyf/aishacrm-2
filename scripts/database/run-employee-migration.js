...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    'Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY. Run via: doppler run -- node scripts/database/run-employee-migration.js',
  );
  process.exit(1);
}
...
async function runMigration() {
  try {
    ...
    const migrationPath = join(
      ...
      'backend',
      'migrations',
      '011_add_employee_phone_department.sql',
    );
