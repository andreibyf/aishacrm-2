...
const BASE_URL =
  ...
  'http://localhost:4000';
const BACKEND_URL =
  process.env.VITE_AISHACRM_BACKEND_URL || process.env.PLAYWRIGHT_BACKEND_URL || '';
...
async function waitForBackendHealth(request) {
  if (!BACKEND_URL) {
    throw new Error(
      'BACKEND_URL is not set. Provide VITE_AISHACRM_BACKEND_URL for non-local runs.',
    );
  }
...
async function ensureUserTenantAssigned(page, email) {
  ...
  try {
    ...
    if (!allowMutations) {
      console.warn(
        `[E2E] Skipping tenant assignment mutation for ${email} on non-local backend (${BACKEND_URL}).`,
      );
...
test.describe('[CRM] CRUD Operations - End-to-End', () => {
    ...
    if (!userInjected) {
      ...
      await page.evaluate(
        ...
        },
        {
          ...
          tId: process.env.E2E_TENANT_ID || '6cb4c008-4847-426a-9a2e-918ad70e7b69',
        },
      );
    }
  });
  test.describe('[CRM] Activities CRUD', () => {
    test('should create a new activity', async ({ page }) => {
      ...
      const addButton = page
        .locator(
          'button:has-text("Add Activity"), button:has-text("New Activity"), button:has-text("Add")',
        )
        .first();
      ...
      await page.waitForSelector('input#subject, [data-testid="activity-subject-input"]', {
        timeout: 8000,
      });
      ...
      const createResp = await Promise.race([
        ...
        page
          .waitForFunction(() => window.__activitySaveSuccess === true, { timeout: 5000 })
          .then(() => ({ ok: () => true })),
      ]).catch(() => null);
      ...
      if (!createResp || !createResp.ok || (createResp.ok && !createResp.ok())) {
        ...
        const errorToastText = await page
          ...
          .catch(() => '');
        throw new Error(
          `Activity creation did not succeed. ${errorToastText || 'No response or save flag.'}`,
        );
      }
      ...
      await page
        .waitForSelector(
          '[data-testid="activity-form"], [data-testid="lead-form"], [data-testid="contact-form"], dialog form',
      ...
      const searchBox = page.locator(
        'input[placeholder*="Search activities"], input[placeholder*="Search"]',
      );
    ...
    test('should edit an existing activity', async ({ page }) => {
      ...
      // Wait for form
      await page.waitForSelector('[data-testid="activity-form"], form.space-y-6', {
        state: 'visible',
      });

      // Get current subject and modify it
      const subjectInput = page.locator(
        'input[name="subject"], input#subject, [data-testid="activity-subject-input"]',
      );
      ...
      await page
        .waitForSelector('[data-testid="activity-form"], form.space-y-6', {
          state: 'hidden',
          timeout: 8000,
        })
        .catch(() => {});
            ...
            async () => {
              try {
                const res = await page.request.get(
                  `${BACKEND_URL}/api/activities/${updatedId}?tenant_id=${tenantIdForPoll}`,
                  { timeout: 5000 },
                );
                if (!res.ok()) return 'pending';
                const data = await res.json();
                return data?.data?.subject && data.data.subject.includes(updatedSubject)
                  ? 'ok'
                  : 'pending';
              } catch {
                return 'pending';
      ...
      const searchBox = page
        .locator(
          'input[placeholder*="Search activities" i], input[placeholder*="Search" i], input[type="search"]',
        )
        .first();
      if ((await searchBox.count()) > 0) {
    ...
    test('should delete an activity', async ({ page }) => {
      ...
      const addButton = page
        .locator('button:has-text("Add Activity"), button:has-text("Add")')
        .first();
      await addButton.waitFor({ state: 'visible', timeout: 8000 });
      await addButton.click();
      await page.waitForSelector('input#subject, [data-testid="activity-subject-input"]', {
      ...
      await page
        .waitForSelector('[role="alertdialog"]', { state: 'hidden', timeout: 5000 })
        .catch(() => {});
    ...
    test('should validate required fields', async ({ page }) => {
      ...
      await page.waitForSelector(
        '[data-testid="activity-form"], form:not([data-testid="ai-sidebar-root"] form)',
        { state: 'visible' },
  ...
  test.describe('[CRM] Leads CRUD', () => {
    test('should create a new lead', async ({ page }) => {
      ...
      const addLeadButton = page
        .locator('button:has-text("Add Lead"), button:has-text("New Lead")')
        .first();
    ...
    test('should update lead job_title without date errors', async ({ page }) => {
      ...
      await expect(page.locator(`table tbody tr:has-text("${testEmail}")`).first()).toBeVisible({
        timeout: 8000,
      });
      ...
      const editBtn = leadRow
        .locator('button')
        .filter({
          has: page.locator('svg.lucide-pencil, svg[class*="edit"], svg.lucide-square-pen'),
        })
        .first();
      ...
      await page
        .waitForSelector('[data-testid="lead-form"]', { state: 'hidden', timeout: 5000 })
        .catch(() => {});
  ...
  test.describe('[CRM] Contacts CRUD', () => {
    test('should create a new contact', async ({ page }) => {
      ...
      const addContactBtn = page
        .locator('button:has-text("Add Contact"), button:has-text("New Contact")')
        .first();
      ...
      if (!contactVisible) {
        ...
        const errorToast = await page
          .locator('[role="alert"]:has-text("Failed"), [role="status"]:has-text("Error")')
          .count();
        if (errorToast > 0) {
    ...
    test('should load contact tags without tenant_id errors', async ({ page }) => {
      ...
      const createRace = Promise.race([
        page.waitForResponse(
          (response) =>
            response.url().includes('/api/contacts') && response.request().method() === 'POST',
          { timeout: 10000 },
        ),
  ...
  test.describe('[CRM] Opportunities CRUD', () => {
    test('should create a new opportunity', async ({ page }) => {
      ...
      const addOppBtn = page
        .locator('button')
        .filter({ hasText: /Add.*(?:Opportunity|Sales|Deal|Pipeline)/i })
        .first();
      ...
      const savePromise = Promise.race([
        page
          .waitForResponse(
            (resp) =>
              resp.url().includes('/api/opportunities') && resp.request().method() === 'POST',
            { timeout: 15000 },
          )
          .then(() => 'network'),
        page
          .waitForFunction(() => window.__opportunitySaveSuccess === true, { timeout: 15000 })
          .then(() => 'flag'),
      ]);
      ...
      await savePromise.catch(async () => {
        const errorToastText = await page
          ...
          .catch(() => '');
        throw new Error(
          `Opportunity creation did not succeed. ${errorToastText || 'No response or save flag.'}`,
        );
      });

      // Wait for opportunity form to close
      await page
        .waitForSelector('form:has(#opp-name)', { state: 'hidden', timeout: 8000 })
        .catch(() => {});
      ...
      const refreshBtn = page
        .locator('button[aria-label*="efresh"], button:has-text("Refresh")')
        .first();
  ...
  test.describe('[CRM] System Logs CRUD', () => {
    test('should create test log and clear all', async ({ page }) => {
      ...
      const settingsLink = page.locator(
        'a[href*="/Settings"]:has-text("Settings"), a[href*="/settings"]:has-text("Settings")',
      );
      ...
      await expect(
        page.locator('h1:has-text("System Logs"), [data-testid="system-logs"]'),
      ...
      if (clearEnabled) {
        ...
        if (deleteVisible) {
          ...
          await Promise.race([
            page
              .locator('text=/Deleted .* log/i')
              .first()
              .waitFor({ state: 'visible', timeout: 8000 })
              .catch(() => {}),
            page
              .locator('text=/No logs matched the filter/i')
              .first()
              .waitFor({ state: 'visible', timeout: 8000 })
              .catch(() => {}),
            page
  ...
  test.describe('[CRM] Data Type Validation', () => {
    test('should enforce priority ENUM values', async ({ page }) => {
      ...
      await expect(
        page.locator('table tbody tr').filter({ hasText: 'Priority Test' }).first(),
      ...
      const hasValidPriorities = await Promise.race(
        validPriorities.map((priority) =>
          page
            .locator(`[role="option"]:has-text("${priority}")`)
            .first()
            .isVisible()
            .catch(() => false),
        ),
