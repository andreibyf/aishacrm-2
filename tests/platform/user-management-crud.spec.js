...
test.describe('[PLATFORM] User Management - Basic CRUD Operations', () => {
  ...
  test('Can access User Management page', async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Add User"), button:has-text("Invite User")'),
  ...
  test('Can see list of existing users', async ({ page }) => {
    ...
    await page.waitForSelector('table, [role="table"], .user-list, [data-testid="users-list"]', {
      timeout: 10000,
    });
  ...
  test('CREATE: Can create a new basic user', async ({ page }) => {
    ...
    await page
      .locator('button:has-text("Add User"), button:has-text("Invite User")')
      .first()
      .click();
    ...
    const roleSelect = page
      .locator('select[name="role"], [role="combobox"]:has-text("Role")')
      .first();
    ...
    await page
      .locator(
        'button:has-text("Send Invite"), button:has-text("Create User"), button[type="submit"]',
      )
      .first()
      .click();
    ...
    const dialogCloseButton = page.locator(
      '[role="dialog"] button:has-text("Cancel"), [role="dialog"] button[aria-label="Close"]',
    );
  ...
  test('READ: Can view user details', async ({ page }) => {
    ...
    const firstUserRow = page
      .locator(
        `table tr:has-text("${SUPERADMIN_EMAIL}"), .user-list-item:has-text("${SUPERADMIN_EMAIL}")`,
      )
      .first();
  ...
  test('UPDATE: Can edit user information', async ({ page }) => {
    ...
    await page.waitForTimeout(1000);
    const userRow = page
      .locator(`table tr:has-text("${testEmail}"), .user-list-item:has-text("${testEmail}")`)
      .first();
    await userRow.click();
    ...
    if (await editButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const nameInput = page
        .locator('input[placeholder*="full name" i], input[placeholder*="name" i]')
        .first();
