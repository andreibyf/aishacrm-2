...
async function getOrCreateTenantB(request) {
  ...
  if (listRes.ok()) {
    ...
    if (Array.isArray(tenants)) {
      const otherTenant = tenants.find((t) => t.id !== TENANT_A_ID && t.slug !== 'system');
      if (otherTenant) {
...
async function setModuleEnabled(request, tenantId, moduleName, enabled) {
  ...
  if (!res.ok()) {
    const text = await res.text();
    throw new Error(
      `Failed to set module ${moduleName} to ${enabled} for tenant ${tenantId}: ${text}`,
    );
  }
}
...
test.describe('Module Settings API Tenant Isolation', () => {
  ...
  test('enabling module for Tenant A does NOT affect Tenant B disabled state', async ({
    request,
  }) => {
