...
test.describe('User Management - Permission System', () => {
  ...
  test('Add User dialog shows all 4 roles for SuperAdmin', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/You can assign:.*superadmin.*admin.*manager.*employee/i'),
  ...
  test('Enable All button toggles all navigation permissions', async ({ page }) => {
    ...
    const dashboardToggle = page
      .locator('button[role="switch"]:near(label:has-text("Dashboard"))')
      .first();
    const contactsToggle = page
      .locator('button[role="switch"]:near(label:has-text("Contacts"))')
      .first();
  ...
  test('Audit logs are created for user creation', async () => {
    ...
    try {
      ...
      const userCreationLog = logs.find(
        (log) => log.source === 'user_management' && log.message?.includes(testEmail),
      );
