...
export async function waitForUserPage(page, email = 'e2e@example.com', tenantId) {
  ...
  const effectiveTenantId =
    tenantId || process.env.E2E_TENANT_ID || '6cb4c008-4847-426a-9a2e-918ad70e7b69';
  await page.evaluate(
    ({ userEmail, tId }) => {
  ...
  if (spinnerGone) {
    ...
    const hasContent = await page
      .locator('table, button:has-text("Add"), button:has-text("New")')
      .first()
    ...
    if (hasContent) {
      ...
      await page
        .waitForSelector('[class*="animate-spin"]', { state: 'hidden', timeout: 15000 })
        .catch(() => {
