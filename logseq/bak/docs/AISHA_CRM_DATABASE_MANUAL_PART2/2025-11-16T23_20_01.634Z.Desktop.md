...
- Emitted events:
  - `aicampaign.created` — After campaign creation. Payload: `{ id, name, status }`.
  - `aicampaign.start` — After scheduling start. Payload: `{ id, status, type, counts: { totalTargets } }`.
  - `aicampaign.pause` — After pausing. Payload: `{ id, status }`.
  - `aicampaign.resume` — After resume. Payload: `{ id, status }`.
  - `aicampaign.progress` — During execution (emitted every 10 contacts). Payload: `{ id, status, progress: { total, processed, success, failed } }`.
  - `aicampaign.completed` — After successful completion. Payload: `{ id, status: 'completed', progress, details }`.
  - `aicampaign.failed` — After execution failure. Payload: `{ id, status: 'failed', progress, details: { error } }`.
...
Campaign Worker (Background Execution)
- Enable background execution by setting `CAMPAIGN_WORKER_ENABLED=true` in backend env (default: `false`).
- The worker polls for campaigns with `status='scheduled'` every `CAMPAIGN_WORKER_INTERVAL_MS` milliseconds (default: `30000` = 30 seconds).
- Architecture:
  - Uses Postgres advisory locks (`pg_try_advisory_lock`) to prevent duplicate processing when multiple backend instances run (e.g., Kubernetes horizontal scaling).
  - Lock ID is computed by hashing the campaign UUID to a 32-bit integer.
  - Sets `status='running'` and stamps `metadata.lifecycle.started_at` before execution.
  - Tracks progress in `metadata.progress` as `{ total, processed, success, failed }`.
  - Updates final status to `completed` or `failed` and stamps `metadata.lifecycle.completed_at` or `failed_at`.
- Execution Flow:
  - Email campaigns: Load tenant-owned sending profile by `ai_email_config.sending_profile_id`, fetch credentials from `tenant_integrations`, personalize template, and send per contact. Supported providers: Gmail, Outlook/Microsoft Graph, Webhook (stub implementations).
  - Call campaigns: Load tenant-owned call integration by `ai_call_integration_id`, fetch credentials, and trigger AI call per contact. Supported providers: CallFluent, Thoughtly (stub implementations).
  - Progress webhooks emitted every 10 contacts; final webhook emitted on completion or failure.
- Tenant Validation: Worker verifies integration ownership (tenant_id match) server-side; cross-tenant execution is rejected.
- Error Handling: Non-blocking; if a campaign fails to process, it is marked `failed` with error logged in `metadata.error`, and the worker continues with the next campaign.
- Environment Variables:
  - `CAMPAIGN_WORKER_ENABLED` — Set to `true` to enable worker (default `false`).
  - `CAMPAIGN_WORKER_INTERVAL_MS` — Polling interval in milliseconds (default `30000`).
- Logging: Worker logs include `[CampaignWorker]` prefix for easy filtering in production logs.
