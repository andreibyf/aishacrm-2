...
test.describe('CRUD Operations - End-to-End', () => {
  ...
  test.describe('Leads CRUD', () => {
    ...
    test('should update lead job_title without date errors', async ({ page }) => {
      ...
      // Wait for success toast to disappear (wait up to 10 seconds)
      const toastLocator = page.locator('div[open][role="status"], div[data-state="open"][role="status"], .toast, [class*="toast"]');
      const toastVisible = await toastLocator.isVisible().catch(() => false);
      if (toastVisible) {
        await toastLocator.waitFor({ state: 'hidden', timeout: 10000 }).catch(() => {
          console.log('[Test] Toast did not disappear, continuing anyway');
        });
        await page.waitForTimeout(500); // Extra buffer after toast disappears
      }
      ...
      const buttonDisabled = await page.locator('button[type="submit"]:has-text("Update")').isDisabled();
      console.log('[Test] Update button disabled:', buttonDisabled);
      
      const buttonVisible = await page.locator('button[type="submit"]:has-text("Update")').isVisible();
      console.log('[Test] Update button visible:', buttonVisible);
      
      // Click the update button (removed force option to see real error)
      await page.click('button[type="submit"]:has-text("Update")');
      console.log('[Test] Clicked Update button');
