...
export async function archiveSessionByIds(tenantId, userId, sessionId) {
  ...
  const tenantUUID = isUuid(tenantId) ? tenantId : (process.env.SYSTEM_TENANT_ID || '00000000-0000-0000-0000-000000000000');
  ...
  if (events.length > 0) {
    const rows = events.map(ev => ({ tenant_id: tenantUUID, user_id: userId, session_id: sessionId, event: ev }));
