...
/**
 * Middleware to block Employee from accessing limited settings routes
...
export function requireManagerRole(req, res, next) {
  ...
  // Superadmin, Admin, and Manager can access webhooks/API keys
  // Employee cannot
  if (user.role === 'employee') {
    return res.status(403).json({ 
      status: 'error', 
      message: 'Access denied. This feature requires manager or administrator privileges.' 
    });
  }
