// Contact entity with bulkAssign extension
...
export const Contact = {
  ...
  async bulkAssign(ids, assignedTo, tenantId, { overrideTeam = false } = {}) {
    ...
    if (!tenantId) throw new Error('tenantId is required for Contact.bulkAssign');
    const authOpts = await getAuthFetchOptions();
    const response = await fetch(`${BACKEND_URL}/api/v2/contacts/bulk-assign`, {
      method: 'POST',
      ...authOpts,
      body: JSON.stringify({
        ids,
