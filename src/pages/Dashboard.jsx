...
export default function DashboardPage() {
  ...
  const widgetPreferencesRef = useRef(widgetPreferences);
  widgetPreferencesRef.current = widgetPreferences;
  ...
  const labeledWidgets = useMemo(
    () =>
      ALL_WIDGETS.map((w) =>
        dynamicWidgetNames[w.id] ? { ...w, name: dynamicWidgetNames[w.id] } : w,
      ),
    [dynamicWidgetNames],
  );
      ...
      try {
        ...
        if (!forceRefresh) {
          ...
          if (cached?.data) {
            ...
            (async () => {
              try {
                const currentPrefs = widgetPreferencesRef.current;
                const visibleWidgetIds = Object.entries(currentPrefs)
        ...
        const isAuthRace =
          ...
          /Authentication required/i.test(String(error));
  ...
  const visibleWidgets = useMemo(() => {
    ...
    if (Object.keys(widgetPreferences).length === 0 && user) {
      ...
      widgets = ALL_WIDGETS.filter((widget) =>
        typeof widgetPreferences[widget.id] !== 'undefined'
  ...
  const handleDragEnd = useCallback(
    async (event) => {
      ...
      const hiddenWidgetIds = ALL_WIDGETS.filter(
        (w) => !visibleWidgets.some((v) => v.id === w.id),
      ).map((w) => w.id);
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                {visibleWidgets.length > 0 ? (
                  visibleWidgets.map((widget, _index) => {
                    ...
                    const prefetchProps = {};
                    if (
                      widget.id === 'recentActivities' &&
                      Array.isArray(bundleLists?.recentActivities)
                    ) {
                    ...
                    if (
                      widget.id === 'salesPipeline' &&
                      Array.isArray(bundleLists?.recentOpportunities)
                    ) {
                     ...
                     */
                    if (
                      widget.id === 'leadSourceChart' &&
                      Array.isArray(bundleLists?.recentLeads)
                    ) {
