/**
 * Activities.jsx Refactoring Smoke Tests
 *
 * Run: npx vitest run src/pages/__tests__/Activities.smoke.test.jsx
 */
...
vi.mock('@/api/entities', () => ({
  Activity: {
    ...
    filter: vi.fn().mockResolvedValue({ activities: [], total: 0, counts: {} }),
...
vi.mock('@/components/shared/entityLabelsHooks', () => ({
  useEntityLabel: () => ({ singular: 'Activity', plural: 'Activities' }),
}));
...
vi.mock('@/components/shared/TimezoneContext', () => ({
  useTimezone: () => ({ selectedTimezone: 'America/New_York' }),
}));

vi.mock('@/components/shared/timezoneUtils', () => ({
  getCurrentTimezoneOffset: vi.fn().mockReturnValue(-300),
  utcToLocal: vi.fn().mockReturnValue(new Date()),
}));
...
const { default: ActivitiesPage } = await import('../Activities.jsx');
...
describe('Phase 3: Stats cards', () => {
  it('stats cards include Total Activities label', async () => {
    render(<ActivitiesPage />);
    await waitFor(() => {
      expect(screen.getByText(/Total Activities/i)).toBeTruthy();
    });
  });
});
