...
function LeadProfilePageContent() {
      ...
      try {
        ...
        try {
          ...
          if (notesRes.ok) {
            ...
            const rawNotes =
              notesData.data?.notes || notesData.notes || notesData.data || notesData;
        ...
        try {
          ...
          if (oppsUrl.includes('_id=')) {
            ...
            if (oppsRes.ok) {
              ...
              const rawOpps =
                oppsData.data?.opportunities || oppsData.opportunities || oppsData.data || oppsData;
        ...
        try {
          if (data.opportunities && data.opportunities.length > 0) {
            ...
            if (import.meta.env.DEV)
              console.log('[Profile] Added opportunity activities:', oppActivities.length);
          }
        } catch (e) {
        ...
        if (
          effectiveTenantId &&
          (!data.ai_summary || shouldRefreshSummary(data.ai_summary_updated_at))
        ) {
          try {
            if (import.meta.env.DEV)
              console.log('[Profile] Calling AI summary endpoint...', {
                entityId,
            ...
            if (summaryRes.ok) {
              ...
              if (ai_summary) {
                ...
                if (import.meta.env.DEV)
                  console.log('[Profile] AI summary stored:', ai_summary.substring(0, 100));
              }
            } else {
              const errText = await summaryRes.text().catch(() => '');
              if (import.meta.env.DEV)
                console.error('[Profile] AI summary endpoint error:', summaryRes.status, errText);
            }
          } catch (e) {
          ...
          if (import.meta.env.DEV)
            console.log('[Profile] Skipping AI summary:', {
              has_tenant: !!effectiveTenantId,
              has_summary: !!data.ai_summary,
              needs_refresh: data.ai_summary
                ? shouldRefreshSummary(data.ai_summary_updated_at)
                : 'N/A',
            });
        }
  ...
  if (error || !profile) {
    const debugInfo = profile
      ? `Profile keys: ${Object.keys(profile).length}`
  ...
  function StageBadge({ stage }) {
    ...
    return (
      <span className={`inline-block px-2 py-1 text-xs font-semibold rounded capitalize ${cls}`}>
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <aside className="fixed top-0 left-0 bottom-0 w-64 bg-gray-900 text-white p-6 overflow-y-auto">
        ...
        <nav className="border-t border-gray-700 pt-4" aria-label="Profile sections">
          <a href="#overview" className="text-white block py-3 text-sm font-medium">
            Overview
          </a>
          <button
          ...
          >
            <span>+ Notes</span>
            {notesCount > 0 && (
              <span className="bg-gray-700 group-hover:bg-gray-600 text-gray-300 text-xs px-2 py-0.5 rounded-full">
                {notesCount}
              </span>
            )}
          </button>
                ...
                {lead.activities.map((activity) => (
                  <div key={activity.id} className="pb-4 border-b border-gray-200 last:border-b-0">
                    ...
                    <div className="text-xs text-gray-600 space-y-1">
                      <p>
                        Type: <span className="font-medium">{activity.type}</span> | Priority:{' '}
                        <span className="font-medium">{activity.priority || 'Normal'}</span>
                      </p>
                      {activity.due_date && (
                        <p>
                          Due:{' '}
                      ...
                      {activity.assigned_to_name && (
                        <p>
                          Assigned to:{' '}
                          <span className="font-medium">{activity.assigned_to_name}</span>
                        </p>
                      )}
                    </div>
                  </div>
