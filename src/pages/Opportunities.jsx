...
export default function OpportunitiesPage() {
  ...
  useEffect(() => {
    if (user && supportingDataReady) {
      // Only load total stats after supporting data is loaded
      loadTotalStats();
    }
  ...
  ); // Added viewMode dependency

  // Load opportunities when dependencies change
  useEffect(() => {
    if (user && supportingDataReady) {
      // Ensure supporting data is loaded before loading opportunities
  ...
  const handleBulkStageChange = async (newStage) => {
    if (selectAllMode) {
      const confirmed = await confirm({
        ...
        description: `Update stage for ALL ${totalItems} opportunity/opportunities matching current filters to ${newStage.replace(
          /_/g,
          ' ',
        )}?`,
        variant: 'default',
  ...
  const handleStageChange = async (opportunityId, newStage) => {
    try {
      logDev('[Opportunities] handleStageChange:', {
        opportunityId,
        newStage,
      ...
      <div className="space-y-6">
        ...
        {isDetailOpen && detailOpportunity && (
          <OpportunityDetailPanel
            ...
            onEdit={(opp) => {
              logDev('[Opportunities] Edit clicked from detail panel:', opp.id);
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="outline"
                  onClick={() => {
                    logDev('[Opportunities] View mode button clicked, current:', viewMode);
                    ...
                    {opportunities.map((opp) => (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {getCardLabel(stageToCardId[opp.stage]) ||
