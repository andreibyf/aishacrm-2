...
export default function ContactsPage() {
  ...
  useEffect(() => {
    ...
    const loadSupportingData = async () => {
      ...
      try {
        ...
        logger.info(
          "Supporting data for contacts loaded successfully.",
          "ContactsPage",
          {
        ...
        logger.error(
          "Failed to load supporting data for contacts",
          "ContactsPage",
          {
            error: error.message,
            stack: error.stack,
  ...
  const loadContacts = useCallback(async () => {
    ...
    try {
      ...
      logger.info(
        "Contacts loaded and paginated successfully.",
        "ContactsPage",
        {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            onClick={() => {
              ...
              logger.info(
                "Opened CSV import dialog for contacts",
                "ContactsPage",
                { userId: user?.id || user?.email },
              );
            }}
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-700">
                  {contacts.map((contact) => {
                    ...
                    const assignedEmployee = employeeMap.get(
                      contact.assigned_to,
                    );
                    const assignedName =
                      assignedEmployee?.first_name &&
                        ...
                        : assignedUser?.full_name || contact.assigned_to_name ||
                          null;
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <Badge
                            variant="outline"
                            className={`${
                              statusBadgeColors[contact.status] ||
                              statusBadgeColors.default
              ...
              (
                <Button
                  onClick={() => {
                    ...
                    setIsFormOpen(true);
                    logger.info(
                      "Opened create new contact form from empty state",
                      "ContactsPage",
                      { userId: user?.id || user?.email },
                    );
                  }}
      ...
      <ContactDetailPanel
        ...
        accountName={accountMap.get(detailContact?.account_id)?.name ||
          detailContact?.account_name}
        assignedUserName={detailContact
          ? employeeMap.get(detailContact.assigned_to)?.first_name &&
            ...
            : userMap.get(detailContact.assigned_to)?.full_name ||
              detailContact.assigned_to_name || null
          : null}
        open={isDetailOpen}
        onOpenChange={(open) => {
      ...
      <CsvImportDialog
        ...
        onImportComplete={() => {
          ...
          loadTotalStats();
          logger.info(
            "CSV import complete, refreshing contacts",
            "ContactsPage",
            { userId: user?.id || user?.email, tenantId: selectedTenantId },
          );
        }}
      />
