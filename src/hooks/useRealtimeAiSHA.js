...
const shouldConsoleLogTelemetry = () => {
  try {
    if (
      ...
      window?.localStorage?.getItem('ENABLE_REALTIME_TELEMETRY_LOGS') === 'true'
    ) {
      return true;
    }
  } catch {
...
const flattenRealtimeContent = (input) => {
  ...
  if (Array.isArray(input)) {
    return input
      .map((chunk) => flattenRealtimeContent(chunk))
      .filter(Boolean)
      .join('');
  }
...
export function useRealtimeAiSHA({ onEvent, telemetryContext } = {}) {
  ...
  const logTelemetryMetric = useCallback(
    (metric, payload = {}) => {
      const eventName = `realtime.telemetry.${metric}`;
      logEvent(eventName, payload);
      if (
        consoleTelemetryEnabledRef.current &&
  ...
  const connectRealtime = useCallback(
    async (options = {}) => {
      ...
      if (!state.isSupported) {
        const details = createErrorDetails('general', {
          message: 'Realtime voice is not supported in this browser.',
        });
      ...
      try {
        ...
        console.log(
          '[useRealtimeAiSHA] Response status:',
        ...
        pc.onconnectionstatechange = () => {
          ...
          if (pc.connectionState === 'failed') {
            const details = createErrorDetails('connection_failed', {
        ...
        const bindDataChannelHandlers = (channel) => {
          if (!channel) return;
          console.log(
            '[Realtime] Binding data channel handlers, channel state:',
            channel.readyState,
          );
          channel.onopen = () => {
          ...
          channel.onmessage = async (event) => {
            ...
            if (pendingResponseLatencyRef.current && isAssistantResponsePayload(payload)) {
              const latencyMs = Math.max(
                0,
                Math.round(getNow() - pendingResponseLatencyRef.current),
              );
              pendingResponseLatencyRef.current = null;
              logTelemetryMetric('model_latency', {
                latencyMs,
                trigger: payload?.type || payload?.role || 'unknown',
              });
            }
            ...
            if (
              ...
              try {
                ...
                // Do not execute the tool with malformed/incomplete args — bail out
                return;
              }
              ...
              if (toolName) {
                ...
                if (executedToolCallsRef.current.has(callId)) {
                  console.log(
                    `[Realtime] Skipping duplicate tool call: ${toolName} (callId: ${callId})`,
                  );
                ...
                if (currentIteration > MAX_TOOL_ITERATIONS) {
                  console.warn(
                    `[Realtime] Tool iteration limit exceeded (${currentIteration}/${MAX_TOOL_ITERATIONS}), skipping tool: ${toolName}`,
                  );
                  logEvent('realtime.tool.iteration_limit_exceeded', {
                    toolName,
                    callId,
                  ...
                  const limitPayload = {
                    ...
                    item: {
                      ...
                      output: JSON.stringify({
                        error:
                          'Tool call limit reached for this request. Please summarize the information you have gathered so far.',
                ...
                try {
                  ...
                  if (channel.readyState === 'open') {
                    ...
                    console.log(
                      `[Realtime] Sent tool result for ${toolName} (iteration ${currentIteration}/${MAX_TOOL_ITERATIONS})`,
                    );
                    logEvent('realtime.tool.result_sent', {
                      toolName,
                      callId,
                      success: true,
                      iteration: currentIteration,
                    ...
                    logEvent('realtime.tool.result_failed', {
                      toolName,
                      callId,
        ...
        console.log(
          '[useRealtimeAiSHA] Ephemeral key length:',
          ephemeralKey?.length,
          'starts with:',
          ephemeralKey?.substring(0, 20),
        );
        ...
        try {
          ...
          console.log(
            '[useRealtimeAiSHA] OpenAI response status:',
            answerResponse.status,
            answerResponse.statusText,
          );
        } catch (fetchError) {
        ...
        if (!answerResponse.ok) {
          ...
          throw Object.assign(new Error('Failed to start realtime session.'), {
