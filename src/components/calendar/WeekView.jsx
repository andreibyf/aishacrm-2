    ...
    <div className="rounded-lg overflow-hidden border border-slate-800">
      <div className="grid grid-cols-7">
        {days.map((d) => (
          <div
            key={d.toISOString()}
            className={`text-xs uppercase tracking-wide font-semibold p-2 text-center border-b border-slate-800 ${isToday(d) ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300'}`}
          >
      ...
      <div className="grid grid-cols-7 gap-px">
        {days.map((d) => (
          <div
            key={d.toISOString()}
            className={`min-h-[220px] p-2 border border-slate-800 ${isToday(d) ? 'bg-slate-800/50' : 'bg-slate-900'}`}
          >
            <div className="space-y-2">
              {getActs(d).map((a) => (
                ...
                >
                  ...
                  <div className="mt-1 flex flex-wrap gap-1">
                    {a.type && (
                      <span className="text-[10px] text-slate-400 border border-slate-600 rounded px-1">
                        {a.type}
                      </span>
                    )}
                    {a.status && (
                      <span
                        className={`text-[10px] rounded px-1 border ${statusColors[a.status] || 'border-slate-600'}`}
                      >
                        {a.status}
                      </span>
                    )}
              ...
              {getActs(d).length === 0 && (
                <div className="text-[12px] text-slate-500 italic">No activities</div>
              )}
            </div>
          </div>
        ))}
