...
function DayCell({ date, inCurrentMonth, activitiesForDay, onActivityClick }) {
  return (
    <div
      className={`border border-slate-800 min-h-[120px] p-2 ${inCurrentMonth ? 'bg-slate-900 calendar-cell-in' : 'bg-slate-900/70 calendar-cell-out'}`}
    >
      <div className="flex items-center justify-between mb-1">
        <span
          className={`text-xs font-semibold ${isToday(date) ? 'text-accent' : 'text-slate-400'}`}
        >
          {format(date, 'd')}
        </span>
        {isToday(date) && (
          <span className="bg-amber-200 text-sky-600 px-1.5 py-0.5 rounded-full">Today</span>
        )}
      </div>
    ...
    <div className="rounded-lg overflow-hidden border border-slate-800">
      <div className="grid grid-cols-7">
        {weekdays.map((d) => (
          <div
            key={d}
            className="bg-slate-800 text-slate-300 text-xs uppercase tracking-wide font-semibold p-2 text-center border-b border-slate-800"
          >
            {d}
          </div>
        ))}
      </div>
      <div className="divide-y divide-slate-800">{rows}</div>
    </div>
  );
}
