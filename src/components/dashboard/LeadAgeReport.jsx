...
import { Clock, TrendingDown, AlertTriangle, User, RefreshCw } from 'lucide-react';
...
function LeadAgeReport(props) {
    ...
    const load = async () => {
      ...
      try {
        ...
        const leadsWithAge = (activeLeads || []).map((lead) => {
          ...
          if (isNaN(ageInDays) || ageInDays < 0 || ageInDays > 9999) {
            console.warn('LeadAgeReport: Invalid age calculated for lead:', {
              id: lead.id,
  ...
  }, [props, cachedRequest, userLoading, authCookiesReady, refreshKey]);

  // Calculate age distribution
  useEffect(() => {
    const buckets = AGE_BUCKETS.map((bucket) => {
      const leadsInBucket = leads.filter(
        (l) => l.ageInDays >= bucket.min && l.ageInDays <= bucket.max,
      );
      return {
        ...bucket,
        ...
        {leads.length === 0 ? (
          ...
          <div className="space-y-4">
                ...
                <div key={index}>
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1">
                      <div
                        className={`px-3 py-1 rounded-md ${bucket.color} font-medium text-sm border`}
                      >
                        {bucket.label}
                      </div>
                      <div className="flex-1 bg-slate-700 rounded-full h-2 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${leads.length > 0 ? (bucket.count / leads.length) * 100 : 0}%`,
                          }}
                        />
                      </div>
                    </div>
            ...
            <div className="text-center pt-4 border-t border-slate-700">
              ...
              >
                <Link to={createPageUrl('Leads')}>View All Active {leadsLabel}</Link>
              </Button>
            </div>
          </div>
        )}
