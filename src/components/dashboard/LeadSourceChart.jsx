  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (
          mounted &&
          props?.stats?.leadsBySource &&
          typeof props.stats.leadsBySource === 'object'
        ) {
        ...
        if (mounted && Array.isArray(props?.leadsData) && props.leadsData.length >= 0) {
          ...
          const filtered = props.leadsData.filter((l) => {
            const tenantOk =
              !effectiveFilter.tenant_id || l.tenant_id === effectiveFilter.tenant_id;
        ...
        const leads = await cachedRequest('Lead', 'filter', { filter: effectiveFilter }, () =>
          Lead.filter(effectiveFilter),
        );
      ...
      <CardContent className="p-6">
        {loading ? (
          ...
          <div className="text-center py-8 text-slate-500 h-[300px] flex flex-col justify-center items-center">
            ...
            <p className="text-sm text-slate-500">{leadLabel} sources will appear here</p>
          </div>
        ) : (
                ...
                <Legend
                  ...
                  payload={data.map((item) => {
                    ...
                    return {
                      ...
                      id: item.originalKey,
                      color:
                        item.value > 0 ? COLORS[itemInVisibleIndex % COLORS.length] : '#64748b', // Grey for zero values
                      payload: {
                        name: item.name,
                        value: item.value,
                  ...
                  formatter={(value, entry) => {
                    ...
                    const totalWithData = data.reduce(
                      (sum, currentItem) => sum + (currentItem.value > 0 ? currentItem.value : 0),
                      0,
                    );
                    const percent =
