...
export const TenantProvider = ({ children }) => {
  ...
  const initFoundValue = useRef(false); // Tracks if URL or localStorage had a tenant value
  ...
  const setSelectedTenantId = useCallback(
      ...
      try {
        ...
        try {
          if (sanitized === null) {
            ...
            try {
              localStorage.setItem('tenant_id', sanitized);
            } catch {
              /* ignore */
            }
          }
    ...
    [selectedTenantId, updateUrlTenantParam],
  ); // Include URL updater to satisfy hook deps
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (urlTenant) {
        initFoundValue.current = true; // Mark that init found a value (prevents auto-select race)
      ...
      if (saved === null || saved === 'null' || saved === 'undefined' || saved === '') {
        ...
        try {
          ...
          if (legacy && isValidId(String(legacy))) {
            ...
            try {
              localStorage.setItem('selected_tenant_id', migrated);
            } catch {
              /* ignore */
            }
            logTenantEvent('INFO', 'Migrated tenant selection from legacy key', {
              tenantId: migrated,
            });
            return;
          }
  ...
  useEffect(() => {
    ...
    // Guard against race condition: init effect may have found a URL/localStorage value
    // but the async state update hasn't landed yet, so selectedTenantId is still null
    if (initFoundValue.current) return;
