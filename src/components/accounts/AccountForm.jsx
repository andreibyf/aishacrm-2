...
export default function AccountForm({ account: legacyAccount, initialData, onSubmit, onCancel }) {
  ...
  const filteredTypeOptions = useMemo(() => {
    ...
    return typeOptions
      .filter(
        (option) => isCardVisible(typeCardMap[option.value]) || formData.type === option.value,
      )
      .map((option) => ({
        ...option,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentUser.role === 'superadmin' && selectedTenantId) {
        ...
        toast.error(
          'Cannot save account: Your account is not configured with a tenant. Please contact your administrator.',
        );
      ...
      let payload = sanitizeNumbers({ ...formData, tenant_id: currentTenantId }, [
        'annual_revenue',
        'employee_count',
      ]);
    ...
    <div className="space-y-6">
      {!loading &&
        ...
        currentUser?.role !== 'superadmin' && (
          ...
          >
            ...
            <AlertDescription>
              Your account is not configured with a tenant. You cannot save records. Please contact
              your administrator.
            </AlertDescription>
          </Alert>
        )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <Label htmlFor="annual_revenue" className="text-slate-200">
              Annual Revenue ($)
            </Label>
            <Input
              id="annual_revenue"
          ...
          <div>
            <Label htmlFor="employee_count" className="text-slate-200">
              Employees
            </Label>
            <Input
              id="employee_count"
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={
                isSubmitting ||
                loading ||
                (!currentUser?.tenant_id && !selectedTenantId && currentUser?.role !== 'superadmin')
              }
