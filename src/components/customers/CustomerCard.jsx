...
const accountTypeColors = {
  b2b: 'bg-blue-900/20 text-blue-300 border-blue-700',
  b2c: 'bg-amber-900/20 text-amber-300 border-amber-700',
};

export default function CustomerCard({ customer, assignedUserName, onEdit, onDelete, onViewDetails, onClick, isSelected, onSelect, user }) {
  const isB2B = customer.account_type === 'b2b';
  ...
  return (
    <Card 
      ...
      style={{ borderLeftColor: customer.type === 'customer' ? '#10b981' : customer.type === 'prospect' ? '#3b82f6' : '#8b5cf6' }}
      onClick={onClick}
    >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <CardTitle className="text-lg font-bold text-slate-100">
                  {isB2B ? customer.name : `${customer.first_name} ${customer.last_name}`.trim() || customer.name}
                </CardTitle>
                <Badge 
                  className={`${accountTypeColors[customer.account_type]} border capitalize text-xs`}
                  data-variant="status"
              ...
              <p className="text-sm font-normal text-slate-400 mt-1">
                {isB2B ? formatIndustry(customer.industry) : customer.job_title || (customer.name || 'No company')}
              </p>
            </div>
          </div>
        </div>
      ...
      <CardContent className="flex-grow space-y-4">
          ...
          {isB2B && (
            <>
              <div className="flex items-center gap-2 text-slate-300">
                ...
                {customer.email ? (
                  <a href={`mailto:${customer.email}`} className="hover:text-blue-400" onClick={(e) => e.stopPropagation()}>
                    {customer.email}
                  </a>
                ) : (
              ...
              <div className="flex items-center gap-2 text-slate-300">
                ...
                {customer.website ? (
                  <a href={customer.website} target="_blank" rel="noopener noreferrer" className="hover:text-blue-400" onClick={(e) => e.stopPropagation()}>
        ...
        {/* B2B Stats: Revenue & Employees */}
        {isB2B && (customer.annual_revenue || customer.employee_count) && (
          <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-700">
            {customer.annual_revenue && (
              <div className="text-center">
                ...
                <span className="text-sm font-semibold text-slate-200">
                  ${(customer.annual_revenue / 1000000).toFixed(1)}M
                </span>
              </div>
            )}
            {customer.employee_count && (
              <div className="text-center">
                ...
                <span className="text-sm font-semibold text-slate-200">{customer.employee_count}</span>
              </div>
            )}
          </div>
        )}
        ...
        <div className="flex justify-between items-center pt-2 border-t border-slate-700">
          <Badge 
            className={`${typeColors[customer.type]} contrast-badge border capitalize`}
            data-variant="status"
            data-status={customer.type}
          >
            {customer.type}
          </Badge>
