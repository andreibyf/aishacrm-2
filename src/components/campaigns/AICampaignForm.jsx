...
import { Users, Bot, Target, X, Save, Zap } from 'lucide-react';
...
export default function AICampaignForm({ campaign, onSubmit, onCancel }) {
  ...
  const handleCampaignTypeChange = (value) => {
    ...
    setSelectedContacts([]);
    const filtered =
      value === 'email' ? allContacts.filter((c) => c.email) : allContacts.filter((c) => c.phone);
            ...
            {formData.campaign_type === 'call' && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Select
                    value={formData.call_integration_id || ''}
                    onValueChange={(v) =>
                      setFormData((prev) => ({ ...prev, call_integration_id: v }))
                    }
                  >
                    <SelectTrigger className="bg-slate-800 border-slate-700 text-slate-200">
            ...
            {formData.campaign_type === 'call' ? (
              ...
              <>
                <div>
                  ...
                  <Select
                    value={formData.email_sending_profile_id || ''}
                    onValueChange={(v) =>
                      setFormData((prev) => ({ ...prev, email_sending_profile_id: v }))
                    }
                  >
                    <SelectTrigger className="bg-slate-800 border-slate-700 text-slate-200">
