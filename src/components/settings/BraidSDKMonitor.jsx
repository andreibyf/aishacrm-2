    ...
    <Card className="bg-card/50">
      <CardContent className="pt-4">
        <div className="flex items-start justify-between">
          ...
          <div
            className={`p-2 rounded-lg ${trend === 'up' ? 'bg-green-500/10' : trend === 'down' ? 'bg-red-500/10' : 'bg-muted'}`}
          >
            <Icon
              className={`w-5 h-5 ${trend === 'up' ? 'text-green-500' : trend === 'down' ? 'text-red-500' : 'text-muted-foreground'}`}
            />
          </div>
        </div>
        {trendValue && (
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={`w-2 h-2 rounded-full ${
            (tool.healthStatus || tool.status) === 'healthy'
              ? 'bg-green-500'
              : (tool.healthStatus || tool.status) === 'degraded'
                ? 'bg-yellow-500'
                : (tool.healthStatus || tool.status) === 'warning'
                  ? 'bg-orange-500'
                  : 'bg-red-500'
          }`}
        />
        <div>
          <p className="font-medium text-sm">
            {(tool.name || tool.tool || 'Unknown').replace(/_/g, ' ')}
          </p>
          <p className="text-xs text-muted-foreground">
            {(tool.calls || tool.total || 0).toLocaleString()} calls
          </p>
        </div>
      </div>
      <div className="flex items-center gap-4">
        <div className="text-right">
          ...
          <p className="text-xs text-muted-foreground">
            {tool.avgLatencyMs || tool.avgLatency || 0}ms avg
          </p>
        </div>
        <HealthBadge
          status={tool.healthStatus || tool.status}
          score={tool.healthScore || tool.health}
        />
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-green-500">
                    {rt?.hour?.success?.toLocaleString() || 0}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                {tm?.summary && (
                  <>
                    <div className="flex items-center justify-between">
                      ...
                      <HealthBadge
                        status={
                          tm.summary.overallHealth >= 80
                            ? 'healthy'
                            : tm.summary.overallHealth >= 60
                              ? 'degraded'
                              : 'warning'
                        }
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="bg-green-500/10 rounded-lg p-3 text-center">
                        <p className="text-2xl font-bold text-green-500">
                          {tm.summary.healthyCount}
                        </p>
                        <p className="text-xs text-muted-foreground">Healthy</p>
                      </div>
                      <div className="bg-yellow-500/10 rounded-lg p-3 text-center">
                        <p className="text-2xl font-bold text-yellow-500">
                          {tm.summary.degradedCount}
                        </p>
                        <p className="text-xs text-muted-foreground">Degraded</p>
                      </div>
                      <div className="bg-orange-500/10 rounded-lg p-3 text-center">
                        <p className="text-2xl font-bold text-orange-500">
                          {tm.summary.warningCount || 0}
                        </p>
                        <p className="text-xs text-muted-foreground">Warning</p>
                      </div>
                      <div className="bg-red-500/10 rounded-lg p-3 text-center">
                        <p className="text-2xl font-bold text-red-500">
                          {tm.summary.criticalCount}
                        </p>
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base">
                    {(selectedTool.tool || selectedTool.name || 'Unknown Tool').replace(/_/g, ' ')}
                  </CardTitle>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-xl font-bold">
                      {(selectedTool.successRate || 0).toFixed(1)}%
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Total Calls</p>
                    <p className="text-xl font-bold">
                      {(selectedTool.calls || selectedTool.total || 0).toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Avg Latency</p>
                    <p className="text-xl font-bold">
                      {selectedTool.avgLatencyMs || selectedTool.avgLatency || 0}ms
                    </p>
                  </div>
                  <div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                      {selectedTool.recentErrors.map((err, idx) => (
                        <div
                          key={idx}
                          className="text-xs bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded p-2"
                        >
                ...
                {selectedTool.errors > 0 && !selectedTool.recentErrors && (
                  <div className="mt-4 text-sm text-muted-foreground bg-muted/50 rounded p-3">
                    <p>
                      ⚠️ This tool has {selectedTool.errors} error(s) but detailed error information
                      is not available.
                    </p>
                    <p className="mt-1">
                      Check system logs or enable error tracking for more details.
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-3 gap-4 text-sm pt-4 border-t">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold">
                      {selectedTool.dependents?.direct?.length || 0}
                    </p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedTool.dependencies.direct.map((dep, idx) => (
                        <Badge key={dep || idx} variant="secondary">
                          {(dep || 'Unknown').replace(/_/g, ' ')}
                        </Badge>
                      ))}
                    </div>
                  </div>
        ...
        <TabsContent value="graph" className="mt-4">
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>
                {graphData?.totalTools || 0} tools across {graphData?.totalCategories || 0}{' '}
                categories
              </CardDescription>
            </CardHeader>
            <CardContent>
