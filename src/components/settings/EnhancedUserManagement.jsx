...
const UserFormModal = ({ user, tenants, currentUser, onSave, onCancel, moduleSettings = [] }) => {
  ...
  useEffect(() => {
    const loadUserModuleSettings = async () => {
      ...
      try {
        ...
        console.log(
          '[UserFormModal] Loaded module settings for tenant:',
          targetTenantId,
          'count:',
          settings?.length || 0,
          'settings:',
          settings?.map((s) => `${s.module_name}=${s.is_enabled}`).join(', '),
        );
  ...
  const disabledModules = useMemo(() => {
    ...
    const settingsToUse =
      userModuleSettings !== null && userModuleSettings.length > 0
        ? userModuleSettings
        : moduleSettings || [];
    ...
    console.log(
      '[UserFormModal] Module settings loaded:',
      settingsToUse.length,
      'Disabled modules for tenant',
      user?.tenant_id,
      ':',
      Array.from(disabled),
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updateData = {
        ...
        tags: formData.tags,
        permissions: {
          // New nested permissions object for CRM capabilities
      ...
      <DialogContent className="max-w-4xl w-[95vw] sm:w-full max-h-[85vh] overflow-y-auto bg-slate-800 border-slate-700 text-slate-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            />
            <p className="text-xs text-slate-500 mt-1">
              Enter a new password to reset. Minimum 8 characters with uppercase, lowercase, number,
                  ...
                  <SelectItem value="manager">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-slate-400">
                        Can see all records in their tenant
                      </span>
                    </div>
                  </SelectItem>
                </SelectContent>
          ...
          <div className="flex items-center space-x-2">
            ...
            <p className="text-xs text-slate-500 mt-1">Deactivating will prevent login.</p>
          </div>
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {formData.tags.map((tag, index) => (
                <Badge
                  key={index}
                  variant="secondary"
                  className="flex items-center gap-1 bg-slate-700 text-slate-200 border-slate-600"
                >
                  {tag}
                  <button
                    type="button"
          ...
          <div className="flex items-center space-x-2">
            <Switch
              ...
              onCheckedChange={(checked) =>
                setFormData((prev) => ({ ...prev, can_use_softphone: checked }))
              }
            />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="crm_access" className="text-slate-200">
                CRM Access
              </Label>
            </div>
            <div>
            ...
            <div>
              <Label htmlFor="dashboard_scope" className="text-slate-200">
                Dashboard Data Scope
              </Label>
              <Select
                value={formData.dashboard_scope}
                onValueChange={(value) =>
                  setFormData((prev) => ({ ...prev, dashboard_scope: value }))
                }
              >
                <SelectTrigger
                  id="dashboard_scope"
            ...
            <div>
              <Label htmlFor="manager_employee_id" className="text-slate-200">
                Manager
              </Label>
              <Select
              ...
              <p className="text-xs text-slate-500 mt-1">
                A manager will be able to filter reports to view this employee’s data (and their
                direct team).
              </p>
            </div>
          </div>
          <div className="mt-2">
                  ...
                  <TooltipProvider key={p.key}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        ...
                        >
                          <span
                            className={`text-sm flex items-center gap-1.5 ${isModuleDisabled ? 'text-slate-400' : 'text-slate-200'}`}
                          >
                            {p.label}
                          ...
                          <Switch
                            checked={
                              isModuleDisabled ? false : !!formData.navigation_permissions?.[p.key]
                            }
            ...
            {!canEditPermissions && (
              <p className="text-xs text-slate-500 mt-1">Only Admin can change these.</p>
            )}
          </div>
          <DialogFooter>
            <Button
...
export default function EnhancedUserManagement() {
  ...
  const handleSaveUser = async (userId, data) => {
    try {
      ...
      console.log(
        '[EnhancedUserManagement] Final update payload for User.update:',
  ...
  const toggleSelectAll = () => {
    if (selectedUsers.size === deletableUsers.length && deletableUsers.length > 0) {
      setSelectedUsers(new Set());
    } else {
      setSelectedUsers(new Set(deletableUsers.map((u) => u.id)));
    }
  };

  const handleClearSelection = () => setSelectedUsers(new Set());

  const handleBulkDelete = async () => {
    // Resolve which users we can actually delete
    const usersToDelete = filteredUsers.filter(
      (u) => selectedUsers.has(u.id) && canDeleteUser(currentUser, u),
    );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      if (response.ok && result.data) {
        ...
        setBulkProgress({ current: deleted + skipped, total: ids.length });
        if (skipped === 0) {
          toast.success(`Deleted ${deleted} user${deleted !== 1 ? 's' : ''}`);
        } else {
          toast.warning(`Deleted ${deleted}, skipped ${skipped} of ${ids.length} users`);
        }
      } else {
        toast.error(`Bulk delete failed: ${result?.message || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    if (roleFilter !== 'all') {
      ...
      if (roleFilter === 'superadmin') {
        ...
        matchesRoleFilter =
          user.employee_role === 'manager' && userRole !== 'admin' && userRole !== 'superadmin';
      } else if (roleFilter === 'user') {
        // Regular users (employee role or no role)
        matchesRoleFilter =
          userRole !== 'admin' && userRole !== 'superadmin' && user.employee_role !== 'manager';
      }
    }
    return matchesSearch && matchesRoleFilter;
  });

  const deletableUsers = filteredUsers.filter((u) => canDeleteUser(currentUser, u));
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm">
            ...
            <p className="text-blue-300">
              Admin (App Owner) → Manager (CRM Tenant Visibility) → Employee (CRM Own Records)
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Select value={roleFilter} onValueChange={setRoleFilter}>
            ...
            <SelectContent className="bg-slate-800 border-slate-700">
              ...
              <SelectItem value="superadmin" className="text-slate-200 hover:bg-slate-700">
                Super Admin
              </SelectItem>
              <SelectItem value="admin" className="text-slate-200 hover:bg-slate-700">
                Admin
              </SelectItem>
              <SelectItem value="power-user" className="text-slate-200 hover:bg-slate-700">
                Manager
              </SelectItem>
      ...
      {selectedUsers.size > 0 && (
        <div className="bg-red-900/20 border border-red-700 rounded-lg p-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
              ...
              <TableHeader>
                <TableRow className="border-slate-700 hover:bg-slate-700/50">
                  <TableHead className="w-10 px-4">
                    <Checkbox
                      checked={
                        selectedUsers.size === deletableUsers.length && deletableUsers.length > 0
                      }
                ...
                {filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map((user) => {
                    ...
                    if (!lastActivity && user.is_active) {
                      ...
                      user.is_active &&
                      lastActivity &&
                      new Date(lastActivity) > oneHourAgo
                    ) {
                    ...
                    const isDeletable = canDeleteUser(currentUser, user);

                    return (
                      <TableRow
                        key={user.id}
                        className={`border-slate-700 hover:bg-slate-700/50 ${selectedUsers.has(user.id) ? 'bg-red-900/10' : ''}`}
                      >
                        ...
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-3">
                            <div className="relative">
                              ...
                              <span
                                className={`absolute bottom-0 right-0 block h-2.5 w-2.5 rounded-full ${statusColor} ring-2 ring-slate-800`}
                              />
                            </div>
                            <div>
                          ...
                          {isCreator && (
                            <Badge
                              variant="outline"
                              className="bg-amber-900/30 border-amber-700/50 text-amber-300 text-xs mt-1"
                            >
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {user.tags.map((tag, tagIndex) => (
                                <Badge
                                  key={tagIndex}
                                  variant="outline"
                                  className="text-xs px-1.5 py-0.5 bg-slate-700 border-slate-600 text-slate-300"
                                >
                                  {tag}
                                </Badge>
                              ))}
                        ...
                        <TableCell>
                          <Badge className={getRoleBadgeClass(user)}>{getRoleDisplay(user)}</Badge>
                        </TableCell>
                        <TableCell>
                          {user.tenant_id ? (
                            <Badge
                        ...
                        <TableCell className="hidden lg:table-cell text-slate-300">
                          {lastActivity ? format(new Date(lastActivity), 'PPpp') : 'Never'}
                        </TableCell>
            ...
            <div className="space-y-3 px-6">
              <div className="p-3 bg-slate-900 rounded border border-slate-700 text-sm">
                <div className="text-slate-300">
                  ...
                  {userToDelete.full_name || userToDelete.display_name || 'N/A'}
                </div>
                <div className="text-slate-300">
                  <strong className="text-slate-200">Email:</strong> {userToDelete.email}
                </div>
                <div className="text-slate-300">
                  <strong className="text-slate-200">Role:</strong> {getRoleDisplay(userToDelete)}
                </div>
                <div className="text-slate-300">
                  <strong className="text-slate-200">Tenant:</strong>{' '}
                  {userToDelete.tenant_id || 'No Tenant'}
                </div>
              </div>
      ...
      <AlertDialog open={bulkDeleteConfirmOpen} onOpenChange={setBulkDeleteConfirmOpen}>
        <AlertDialogContent className="bg-slate-800 border-slate-700">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-300">
              Are you sure you want to permanently delete {selectedUsers.size} selected user
              {selectedUsers.size !== 1 ? 's' : ''}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-3 px-6">
            <div className="text-yellow-400 font-semibold text-sm">
              ⚠️ This action cannot be undone. All selected users will be removed from
