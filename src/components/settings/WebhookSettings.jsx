      ...
      <Card className="shadow-lg border-0 bg-slate-800 border-slate-700">
        <CardHeader>
          ...
          <CardDescription className="text-slate-400">
            Use these URLs to send data TO your CRM from external systems like Zapier, Make.com,
            n8n, or custom applications.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Alert className="mb-6 bg-blue-900/30 border-blue-700/50">
            ...
            <AlertDescription className="text-blue-300">
              All incoming webhooks require your `N8N_API_KEY` in the `x-api-key` header for
              authentication.
            </AlertDescription>
          </Alert>
          ...
          <div className="space-y-4">
            <div className="border rounded-lg p-4 bg-slate-700/30 border-slate-600">
              ...
              <p className="text-sm text-slate-400 mb-3">
                Send any CRM data (contacts, leads, accounts, activities) using a unified endpoint.
                Supports both creating and updating records.
              </p>
              <div className="bg-slate-700 p-3 rounded border font-mono text-sm break-all text-slate-200">
                POST {window.location.origin}/api/webhooks/incoming
              </div>
    ...
    "status": "customer"
            ...
            <div className="border rounded-lg p-4 bg-slate-700/30 border-slate-600">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between items-center py-1">
                  ...
                  <code className="text-xs bg-slate-600 px-2 py-1 rounded text-slate-200">
                    /functions/n8nCreateContact
                  </code>
                </div>
                <div className="flex justify-between items-center py-1">
                  <span className="text-slate-300">Create Leads:</span>
                  <code className="text-xs bg-slate-600 px-2 py-1 rounded text-slate-200">
                    /functions/n8nCreateLead
                  </code>
                </div>
                <div className="flex justify-between items-center py-1">
                  <span className="text-slate-300">Call Results:</span>
                  <code className="text-xs bg-slate-600 px-2 py-1 rounded text-slate-200">
                    /functions/thoughtlyCallResults
                  </code>
                </div>
                <div className="flex justify-between items-center py-1">
                  <span className="text-slate-300">Call Transcripts:</span>
                  <code className="text-xs bg-slate-600 px-2 py-1 rounded text-slate-200">
                    /functions/thoughtlyTranscripts
                  </code>
                </div>
              </div>
            </div>
      ...
      <Card className="shadow-lg border-0 bg-slate-800 border-slate-700">
        <CardHeader>
          ...
          <CardDescription className="text-slate-400">
            Notify external systems when events happen in the CRM. Use this to trigger workflows in
            Pabbly, Zapier, Make.com, or n8n when a contact is created, lead is updated, etc.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Alert className="mb-6 bg-blue-900/30 border-blue-700/50">
            <AlertCircle className="h-4 w-4 text-blue-400" />
            <AlertDescription className="text-blue-300">
              All webhooks are digitally signed using your `N8N_SHARED_SECRET` for security (despite
              the name, this works with any webhook service). The signature is sent in the
              `x-webhook-signature` header for optional validation.
            </AlertDescription>
          </Alert>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {webhooks.map((webhook) => (
                <Card
                  key={webhook.id}
                  className="flex items-center justify-between p-4 bg-slate-700 border-slate-600"
                >
                  <div className="flex items-center gap-4">
                    <WebhookIcon
                      className={`w-6 h-6 ${webhook.is_active ? 'text-green-500' : 'text-slate-500'}`}
                    />
                    <div>
