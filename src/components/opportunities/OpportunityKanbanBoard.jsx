...
const stageConfig = {
  prospecting: {
    ...
    cardId: 'opportunity_prospecting',
  },
  qualification: {
    title: 'Qualification',
    color: 'border-t-indigo-700',
    cardId: 'opportunity_qualification',
  },
  proposal: { title: 'Proposal', color: 'border-t-purple-700', cardId: 'opportunity_proposal' },
  negotiation: {
    title: 'Negotiation',
    color: 'border-t-yellow-700',
    cardId: 'opportunity_negotiation',
  },
  closed_won: { title: 'Closed Won', color: 'border-t-emerald-700', cardId: 'opportunity_won' },
  closed_lost: { title: 'Closed Lost', color: 'border-t-red-700', cardId: 'opportunity_lost' },
};
...
export default function OpportunityKanbanBoard({
  ...
  const getDisplayInfo = (opp) => {
    ...
    if (opp.contact_id) {
      ...
      return (
        `${contact?.first_name || ''} ${contact?.last_name || ''}`.trim() || contact?.email || 'N/A'
      );
    }
    if (opp.lead_id) {
          ...
          {visibleStages.map((stage) => {
            ...
            return (
              <div key={stage.id} className="flex flex-col h-full">
                <Card
                  className={`kanban-stage-card bg-slate-800 border border-t-4 border-l-slate-700 border-r-slate-700 border-b-slate-700 ${stage.color} shadow-md mb-4 rounded-lg`}
                >
