    ...
    async (fileToProcess) => {
      ...
      try {
        ...
        if (extractionResult.status === 'success' && extractionResult.output?.transactions) {
          setExtractedData(
            extractionResult.output.transactions.map((t) => ({ ...t, category: 'other' })),
          );
          setStatus('review');
        } else {
          throw new Error(
            extractionResult.details || 'Failed to extract structured data from the document.',
          );
        }
      } catch (err) {
                  ...
                  <TableRow key={index} className="border-slate-700">
                    ...
                    <TableCell>
                      <Input
                        ...
                        value={item.amount}
                        onChange={(e) =>
                          handleRowChange(index, 'amount', parseFloat(e.target.value))
                        }
                        className="bg-slate-700 border-slate-600 text-slate-200"
                      />
                    </TableCell>
