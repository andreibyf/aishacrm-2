...
export default function ResendInviteButton({
  ...
  const handleResend = async () => {
    if (!email || !fullName || !tier || !role || !employeeId) {
      alert("Missing data to resend. Make sure Email, Tier, and Role are set, and save the Employee first.");
      return;
    }
    setSending(true);
    try {
      ...
      if (ok && isAdminLike && !responseData.invitee_email_sent) {
        const appUrl = typeof window !== "undefined" ? window.location.origin : "https://app.base44.io";
        const body = `
