  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        // For existing leads, use their assigned_to value.
        // For new leads, default to unassigned — user can claim via "Assign to me".
        // Managers default to themselves but can change via dropdown.
        let initialAssignedTo = lead?.assigned_to;
        if (!lead) {
          // New lead
          if (isManager && user?.email) {
            initialAssignedTo = user.email; // Manager defaults to self, can change
          }
          // Non-managers: leave unassigned — they can claim it
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!submissionData.tenant_id) {
        ...
        toast.error(
          'Cannot save lead: No client assigned to your account. Please contact an administrator.',
        );
      ...
      if (!lead) {
        // This is a new lead creation
        try {
          const idResponse = await generateUniqueId({
            entity_type: 'Lead',
            tenant_id: submissionData.tenant_id,
          });
        ...
        // This is an existing lead update
        if (lead.unique_id) {
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-6" data-testid="lead-form">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <Label htmlFor="score" className="text-slate-200">
              Lead Score (0-100)
            </Label>
            <Input
              id="score"
        ...
        <div>
          <Label htmlFor="score_reason" className="text-slate-200">
            Score Reason
          </Label>
          <Input
            id="score_reason"
