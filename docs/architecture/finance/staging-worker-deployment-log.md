...
- [ ] Phase 3-3 verification execution per [`staging-rls-verification-results.md`](./staging-rls-verification-results.md) §4 has been completed in staging, with §4.1 (PostgREST exclusion), §4.2 (`auth.role()`), and §4.3.a (scratch-table rehearsal) all PASS. `§4.5` may be deferred if migrations 172/175 have not yet been applied, but the worker app SHOULD NOT be created if any §4 check has FAILED.
- [ ] Phase 3-2 staging migrations applied per [`staging-migration-application-log.md`](./staging-migration-application-log.md) §3 — at minimum 172 + 173 (`finance.audit_events` exists, append-only triggers installed); 174 (`finance.projection_state`) is required for projection-state durability across worker restarts but the worker can run against an empty table.
...
- [ ] Doppler `stg_stg` config carries the secrets the worker needs **to boot at all** — the entry block at `backend/workers/financeProjectionWorker.js:378-383` resolves `FINANCE_DB_URL || DATABASE_URL` and **exits the process if neither is set**, _before_ the disabled gate at line 257 ever runs. A disabled-by-default deploy therefore still needs a valid Postgres connection string in Doppler `stg_stg` (`FINANCE_DB_URL` preferred, with `DATABASE_URL` as the fallback) just to start the container. `DOPPLER_TOKEN` for the worker app's service account is required for the same boot-time reason.
...
Outcome: the Coolify app exists and is in **running, unhealthy, idle (acceptable while disabled)** state. Sequence:

1. The container starts.
2. The entry block (`backend/workers/financeProjectionWorker.js:378-383`) reads `FINANCE_DB_URL` (or `DATABASE_URL`) from Doppler-injected env. **If neither is set, the process exits — see §6.1.**
3. The entry block constructs a `pg.Pool` object (`pg.Pool` construction is lazy — no TCP connection opens at construction time), then constructs a runner and event store referencing the pool.
4. `startFinanceProjectionWorker(...)` is called. `isFinanceProjectionWorkerEnabled()` returns `false` (because `ENABLE_FINANCE_OPS` is unset). The factory logs `[finance-projection-worker] disabled — idling` and returns an idle stub without scheduling a poll timer.
5. **No `pool.query()` is ever called**, so although the `pg.Pool` object exists, no actual TCP connection to Supabase is opened in the disabled state.

**Healthcheck status: `unhealthy` is expected and acceptable in the disabled state.** Per §5.1, the disabled-and-idling code path does NOT write a heartbeat — the disabled return path is a single log line and an idle stub, with no `writeWorkerHeartbeat()` call. The heartbeat-file healthcheck defined in [`finance-workers.staging.example.yml`](../../../deploy/coolify/finance-workers.staging.example.yml) therefore reports `unhealthy` for a disabled worker. **This is the intended Phase 3-4 outcome.** Coolify shows the container as `running` (the container is alive); Docker reports `unhealthy` (the heartbeat file is absent); the operator differentiates "disabled and idling correctly" from "enabled and stuck" by reading the `[finance-projection-worker] disabled — idling` log line and confirming `ENABLE_FINANCE_PROJECTION_WORKER` is unset in the worker app's env. When Phase 3-5 flips the enable flags, the heartbeat file starts being written on every poll cycle and the healthcheck transitions to healthy.

A follow-up to emit a one-time `writeWorkerHeartbeat({ status: 'disabled' })` on the idle return path would make the healthcheck report healthy in the disabled state too — at which point the outcome wording could move to "running, healthy, idle". **Tracked as a follow-up code change, not a 3-4 blocker** — operators have unambiguous log + env signals for the disabled state today.

**Rollback for 6.2:** Delete the Coolify app. No DB state was changed; no actual TCP connection to Supabase was opened (per the sequence above); no data exists to lose.
...
- [ ] No actual DB connection to staging Supabase is opened from the worker container. The `pg.Pool` object is constructed in the entry block (`pg.Pool` construction is lazy — no TCP connection is opened by `new pg.Pool()`), but the disabled gate returns before any `pool.query()` runs, so no connection is made. Validated by absent "connection opened" log lines and (if accessible) by Supabase project's connection-pool metrics showing no new connections from the worker's source IP / `service_role` connection count.
...
| `FINANCE_DB_URL`                           | (injected by Doppler; **required at process startup** — entry block at `financeProjectionWorker.js:378-383` exits if neither this nor `DATABASE_URL` is set, _before_ the disabled gate runs) | Never — Doppler-injected.                                                                                                                             |
