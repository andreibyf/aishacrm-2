...
| Journal posting                                         | **No HTTP path to `posted` under 3-8 conditions.** `approveFinanceAction()` at `backend/lib/finance/financeDomainService.js:596-644` only mutates the approval row (`status`, `approved_by`, `approved_at`); it does NOT transition the target journal from `pending_approval` to `posted`. Under HTTP-only operator actions the journal stays at `pending_approval` indefinitely. See §5.7 "what `approveFinanceAction()` does NOT do" and §5.9.b for the deferral reason. The route tests at `backend/__tests__/routes/finance.v2.routes.test.js:101-123` use `service.seedJournalEntry({ ..., status: 'posted' })` directly to bypass this, but no HTTP equivalent exists.                                        |
| Reversal                                                | **`/journal-entries/:id/reverse` rejects non-posted journals with HTTP 409** per `financeDomainService.js:523-526` (`'Only posted journal entries can be reversed'`). Combined with the no-HTTP-path-to-posted row above, **the reverse-success path is not reachable via HTTP in 3-8** (this is the §5.9.b deferral). The reject path IS testable and is verified by §5.9.a. The success-path contract — when a posted journal does exist (e.g., via the seed-based route tests) — constructs a NEW reversal journal with debit/credit inverted (`accountingEngine.createReversalDraft`), with `reversal_of: <original-id>`, requiring a fresh approval. Original journal stays unchanged; nothing is hard-deleted. |
