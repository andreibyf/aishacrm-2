...
- The **evidence ownership + handoff into Phase 4-19**: who owns which evidence file; how it gets attached to the Phase 4-19 activation decision.
...
- §7: Evidence ownership + Phase 4-19 handoff.
- §8: Hard constraints.
- §9: Acceptance.
...
- Name specific individuals as support owner / security reviewer. Phase 4-15 freezes the role contract; Phase 4-5 (owner matrix population) / Phase 4-0 §9 + the downstream owner-matrix packet name individuals.
...
| **Engagement window**                 | Support owner is "on" from Phase 4-19 GO through stand-down + 7 days post-rollback (or post-pilot completion). The window ends when monitoring owner declares the pilot stable AND security reviewer signs off on the root-cause assessment (if any incident occurred).                                                                                                                                                  |
...
**Phase 4-19 verification** (row 15): customer-side primary contact confirmed reachable for the activation window; support owner confirmed availability for the activation window.
...
| **"Pilot activation complete"**                     | Post-activation, t+1h after Phase 4-19 GO.       | Customer-side primary + technical contacts.                                               | "Activation completed. Console available at {URL}. Watching signals for next 24h."                                                                                            |
...
| **Re-activation decision.**                | A fresh Phase 4-19 GO is required to re-attempt activation. The Phase 4-19 packet consumes the tenant-leakage RCA as input; a tenant-leakage event may result in pilot termination rather than retry.                                                                                                                         |
...
**Definition:** evidence of a provider-side write that was not authorised by a Phase 3-10 §6.6 / Phase 4-14 controlled-flip window. Phase 4-4 signal 11 (provider-write kill switch unauthorised true) is the primary internal indicator; a customer report of unexpected ERPNext records is the primary external indicator.
...
| **Re-activation decision.**              | Audit-log integrity violation typically results in pilot termination. Re-activation requires a fresh Phase 4-19 GO with the integrity-violation RCA + remediation evidence consumed; may also require independent third-party security review (outside this freeze's scope). |

---

## 7. Evidence ownership and Phase 4-19 handoff

Phase 4-19 (the activation decision packet, including re-activation after any rollback or incident) consumes evidence from every Phase 4 packet. Phase 4-15 freezes the evidence ownership matrix:

| Evidence file                                                                                                                                              | Owner                             | Phase 4-19 row consumed                                                                                                                                                                            |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Pilot incident log** — every customer report + every internal alert during the pilot window, with timestamps, classifications, routing, and resolutions. | Support owner.                    | Phase 4-19 row 15 (customer-side primary contact reachable) verified by demonstrating that support owner engaged customer-side contacts; for re-activation, the incident log is the primary input. |
| **Customer communication log** — every outbound message sent to the customer during the pilot window, with timestamps, audience, channel, content.         | Support owner.                    | Phase 4-19 row 14 (customer-side communication contact confirmed) referenced.                                                                                                                      |
| **Security incident record** — per §6 incident, the full playbook execution (steps, timestamps, scope assessment, disclosure timing, RCA, remediation).    | Security reviewer.                | Phase 4-19 row 11 (16 safety guardrails verified still in committed code) — if a security incident occurred, its remediation is part of the guardrail-state assessment.                            |
| **Rollback evidence record** — per Phase 4-5 §6 structure, populated per rollback execution.                                                               | Rollback owner.                   | Phase 4-19 row 13 (regression + security review PASS) consumes rollback evidence for re-activation decisions.                                                                                      |
| **Phase 4-4 signal snapshots** — periodic dashboard snapshots during the pilot window + critical-moment snapshots.                                         | Monitoring owner.                 | Phase 4-19 row 5 (observability runbook committed) verification.                                                                                                                                   |
| **Post-incident RCAs** — within 7 days of any incident, per Phase 4-5 §7.                                                                                  | Deploy owner + security reviewer. | Phase 4-19 re-activation input.                                                                                                                                                                    |
| **Pilot stand-down summary** — at pilot conclusion, a consolidated summary of incidents, customer engagement, lessons learned.                             | Support owner + deploy owner.     | Phase 4-19 row 15 + post-pilot decisions.                                                                                                                                                          |

**The Phase 4-19 packet template includes a §X "evidence consumed" section listing the files above + their owners + their timestamps. Phase 4-19 cannot be authored without those files populated for the pilot window in question.**
...
| **Define evidence ownership and handoff into Phase 4-19 activation decision.**                                                        | Slack directive                        | §7.                                                                                |
...
| **Re-activation after any rollback / incident requires a fresh Phase 4-19 GO.**                                                       | Phase 4-0 §11.2 + §11.3 + Phase 4-5 §7 | §6 + §7.                                                                           |
...
- [x] Evidence ownership defined; Phase 4-19 handoff defined (§7).
