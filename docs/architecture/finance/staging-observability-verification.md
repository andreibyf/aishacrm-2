...
**Summary:** 5 signals are IMPLEMENTED today (1, 3, 8, 9) plus 1 PARTIAL (4 via SQL, 7 via signal 4 for audit_timeline). 4 signals are explicitly DEFERRED (2 needs `/ready`, 5 + 6 need adapter worker, 7 standalone audit pack needs audit worker). No signal is silently absent — every gap is named here.
