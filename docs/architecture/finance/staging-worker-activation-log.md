...
- [ ] **Phase 3-2 migrations applied to staging.** At minimum 172 (`finance` schema + 8 tables) and 173 (`finance.audit_events` append-only triggers); 174 (`finance.projection_state`) is required for projection-state durability across restarts; 175 (RLS policies on all 9 tables + no-hard-delete triggers) is required so the worker's `service_role` reads/writes work as expected and so any future direct PostgREST exposure stays fail-closed. Per [`staging-migration-application-log.md`](./staging-migration-application-log.md).
...
1. `await eventStore.replay(tenantId)` → reads the full ordered tenant stream from `finance.audit_events` via the Postgres adapter. Ordering is `created_at` ASC, `id` ASC (the frozen Track A contract, backed by migration 173's `idx_finance_audit_events_replay (tenant_id, created_at, id)`).
...
- **No schema rollback required.** No migration was applied by 3-5; rolling back the worker doesn't roll back migrations 172/173/174/175 (which 3-2 applied).
