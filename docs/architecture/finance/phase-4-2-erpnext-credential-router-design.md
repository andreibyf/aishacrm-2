...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hostname matches a **built-in sandbox host pattern** at `erpnextSandboxAdapter.js:89-98` — `BUILTIN_SANDBOX_HOST_PATTERNS` is the canonical list: `localhost` / `127.0.0.1` / `0.0.0.0` exact match, or hostname ends with `.local` / `.lan` / `.internal`, or hostname starts with `sandbox.` (e.g., `sandbox.example.com`), or hostname contains `-sandbox.` (e.g., `acme-sandbox.frappe.cloud`). **Note:** a generic vendor host like `customer.frappe.cloud` is NOT a built-in match — it must be enumerated in the env allow-list below. | Router returns the credentials; adapter construction will succeed.                                                                                                                            |
| Hostname listed in `FINANCE_ERPNEXT_SANDBOX_BASE_URLS` env (operator-extended allow-list of exact, fully-qualified hostnames; case-insensitive match)                                                                                                                                                                                                                                                                                                                                                                                         | Router returns the credentials with `sandboxAllowlist` populated from the env; adapter construction will succeed.                                                                             |
| Hostname neither matches a built-in pattern NOR is listed in the env allow-list (e.g., a production-looking ERPNext URL, or a vendor sandbox host that hasn't been added to the env allow-list)                                                                                                                                                                                                                                                                                                                                               | Adapter construction throws `AdapterConfigError`. Router does NOT pre-filter — the adapter constructor is the authoritative gate. Router treats this as "credentials configured but invalid". |
