...
- [ ] **Phase 3-2 migrations applied to staging.** At minimum 172 (`finance` schema + 8 tables including `finance.adapter_jobs`), 173 (`finance.audit_events` append-only triggers), 174 (`finance.projection_state`), and 175 (RLS policies on all 9 tables + no-hard-delete triggers) — so the adapter worker's `service_role` reads/writes against `finance.adapter_jobs` and `finance.audit_events` succeed. Per [`staging-migration-application-log.md`](./staging-migration-application-log.md).
...
- **No schema rollback required.** No migration was applied by 3-9; rolling back the worker doesn't roll back migrations 172/173/174/175.
