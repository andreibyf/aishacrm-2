...
| RLS                 | Intentionally **not enabled** — placeholders only, commented out at the bottom (RLS belongs to migration 175)                                         |
| Touches `public.*`? | No                                                                                                                                                    |
...
### 2.2 Migration 173 — `173_finance_event_store_append_only.sql`

| Field                   | Value                                                                                                                                                                                               |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| File                    | `backend/migrations/173_finance_event_store_append_only.sql`                                                                                                                                        |
...
| Hardens                 | `finance.audit_events` (created in 172) into the append-only persistent event store — the canonical Postgres-backed finance event stream                                                            |
...
| Depends on              | Migration 172 (operates on `finance.audit_events`)                                                                                                                                                  |

### 2.3 Migration 174 — `174_finance_projection_state_draft.sql`

| Field               | Value                                                                                                                                                                                                                                                                                                                             |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| File                | `backend/migrations/174_finance_projection_state_draft.sql`                                                                                                                                                                                                                                                                       |
...
| Append-only?        | **No** — intentionally mutable. `state_json` is a durable cache of derived state, rebuilt by `replay()` from the event stream which remains the source of truth. No no-hard-delete trigger here (see §2.4 — migration 175 also does not add one for this table)                                                                   |
| Touches `public.*`? | No                                                                                                                                                                                                                                                                                                                                |
...
| Depends on          | Migration 172 (depends on `finance` schema)                                                                                                                                                                                                                                                                                       |

### 2.4 Migration 175 — `175_finance_rls_policies.sql`

| Field                       | Value                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| File                        | `backend/migrations/175_finance_rls_policies.sql`                                                                                                                                                                                                                                                                                                                                                                              |
| Header posture              | "Dev-only DRAFT. Do NOT apply to staging/production until the Phase 3-2 staging-readiness review clears."                                                                                                                                                                                                                                                                                                                      |
| Scope                       | **Full finance RLS rollup** — enables Row-Level Security on **all 9** finance tables (8 from 172 + `projection_state` from 174) and installs the finalized policy set and the no-hard-delete ledger triggers                                                                                                                                                                                                                   |
...
| Service_role behavior       | `service_role` bypasses RLS entirely. The no-hard-delete triggers are the authoritative guard against `service_role` deletes on immutable ledger rows; `audit_events` UPDATE/DELETE/TRUNCATE is additionally guarded by the append-only triggers from migration 173.                                                                                                                                                           |
| Touches `public.*`?         | No                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Depends on                  | Migrations 172 and 174 (covers tables created by both)                                                                                                                                                                                                                                                                                                                                                                         |

### 2.5 Migration 175 — naming / scope reconciliation (mismatch flagged)

Migration 175 is the **full finance RLS rollup**, not a narrow companion to 174. Prior docs have under-described it:

| Prior doc                                                   | What it said                                                                                                   | Reality (per the actual file contents)                                                                                    |
| ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `phase-3-staging-activation-plan.md` §6 (commit `cbd1a65c`) | "`175_finance_projection_state_rls_draft.sql` — Companion RLS for `finance.projection_state`. Dev-only draft." | Actual file is `175_finance_rls_policies.sql`. It is the full RLS rollup covering all 9 tables + no-hard-delete triggers. |
| CHANGELOG entry for Slice 1 Task 1 (commit `1b322a3f`)      | "add companion RLS migration 175 (dev-only draft)"                                                             | Same — under-describes scope.                                                                                             |
| Slice 1 omnibus CHANGELOG entry                             | "**Task 1** adds dev-only draft migration `175_finance_projection_state_rls_draft.sql` (RLS companion to 174)" | Same — wrong filename and under-described scope.                                                                          |

**This doc supersedes those characterizations for Phase 3-2 onwards.** Migration 175 is `175_finance_rls_policies.sql` — the full finance RLS rollup. The scaffold is updated alongside this doc to reflect the correct name and scope.
...
1. **172** — `172_finance_ops_runtime_scaffold.sql` — creates `finance` schema and 8 tables.
2. **173** — `173_finance_event_store_append_only.sql` — append-only triggers on `finance.audit_events` + replay index.
3. **174** — `174_finance_projection_state_draft.sql` — creates `finance.projection_state` + updated_at trigger.
4. **175** — `175_finance_rls_policies.sql` — enables RLS on all 9 tables + 35 policies + 5 no-hard-delete triggers.

**Do not skip 173 between 172 and 175.** 173 is what makes `finance.audit_events` append-only; if 175 lands first, the RLS policies are in place but the table is still mutable until 173 lands. The window is harmless on an empty table (no events yet to corrupt), but the canonical order eliminates the ambiguity.

**Do not apply 175 before 174.** 175 includes `enable row level security` on `finance.projection_state` and a SELECT / INSERT / UPDATE policy set for it — that requires the table to exist.
...
- [ ] No uncommitted change to any file under `backend/migrations/17[2-5]_*.sql`. `git status` for the four files must show clean.

### 5.2 Staging-readiness checks (per Phase 2C deliverables)

- [ ] `staging-rls-validation.md` (2B-14) preflight is clear — specifically, the JWT `tenant_id` claim shape has been confirmed against the staging auth setup (the RLS policies in 175 hard-code `auth.jwt() ->> 'tenant_id'`).
...
- [ ] `phase-2c-rls-application-plan.md` (2C-1) §7 readiness checklist is clear for migrations 172/173 application.

### 5.3 Environment posture checks (the hard constraints from Phase 3-1 §7-9)

- [ ] `ENABLE_FINANCE_OPS` is **unset (or not `'true'`)** in the staging backend Doppler config (`stg_stg`) at apply time. Migrations 172-175 are schema changes; the route mount remains gated by `ENABLE_FINANCE_OPS` independently. Setting `ENABLE_FINANCE_OPS=true` is Phase 3-7, not 3-2.
...
- [ ] `select count(*) from pg_namespace where nspname = 'finance'` — record whether the schema already exists. If it does, migrations 172-175 are still safe to re-apply (every statement is idempotent — see §2 idempotency rows), but the operator records the pre-state for the post-apply diff.
- [ ] `select table_name from information_schema.tables where table_schema = 'finance' order by table_name` — record current table set. Expected pre-state for a fresh staging: empty. Expected post-apply: 9 tables (8 from 172 + `projection_state` from 174).
- [ ] `select policyname, tablename from pg_policies where schemaname = 'finance' order by tablename, policyname` — record current policy set. Expected pre-state: empty (no 175 yet). Expected post-apply: 35 rows.
- [ ] `select tgname, tgrelid::regclass from pg_trigger where tgname like '%finance%' or tgrelid::regclass::text like 'finance.%' order by tgrelid::regclass, tgname` — record current triggers. Expected post-apply: at minimum the 3 audit-events immutability triggers from 173, the projection_state updated_at trigger from 174, and 5 no-hard-delete triggers from 175.
- [ ] `select extname from pg_extension where extname = 'pgcrypto'` — confirm `gen_random_uuid()` is available (used as default in every PK in 172). If absent, application fails on table creation.
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- |
| 1    | Drop the 5 no-hard-delete triggers (175); drop the 2 functions `finance.prevent_hard_delete` / `finance.prevent_hard_delete_posted`; drop the 35 RLS policies; `alter table … disable row level security` on each of the 9 finance tables.                                                 | Migration 175 |
| 2    | Drop the `trg_projection_state_set_updated_at` trigger and the `finance.set_projection_state_updated_at` function; drop the `finance_projection_state_cursor_pair_chk` constraint; drop the `idx_finance_projection_state_tenant_status` index; drop the `finance.projection_state` table. | Migration 174 |
| 3    | Drop the 3 audit-events immutability triggers (`trg_audit_events_no_update`, `trg_audit_events_no_delete`, `trg_audit_events_no_truncate`); drop the `finance.audit_events_immutable` function; drop the `idx_finance_audit_events_replay` index.                                          | Migration 173 |
| 4    | Drop the 8 `finance.*` tables (and their indexes by cascade); drop the `finance.validate_journal_entry_balance` function; drop the `finance` schema.                                                                                                                                       | Migration 172 |

For a partial rollback (e.g., 175 misbehaves but 172-174 are kept), execute only step 1.
...
- [x] Migrations 172, 173, 174, 175 inventoried against actual file contents (§2)
- [x] Migration 175 naming / scope mismatch with prior docs flagged and superseded (§2.5)
