...
- `id` uses the `evt_{uuid}` prefix format from `financeEventEnvelope.js`, not a bare UUID. This prevents confusion with aggregate IDs and makes log grep straightforward.
